  ...
  _updateCalledElement(element, oldTemplate, newTemplate) {
    ...
    if (!newProperties.length) {
      commandStack.execute('element.updateModdleProperties', {
        ...
        moddleElement: businessObject,
        properties: {
          values: minDash.without(businessObject.get('values'), calledElement)
        }
      });
      return;
    }
...
let RemoveElementTemplateHandler$1 = class RemoveElementTemplateHandler {
  ...
  _copyExtensionElements(source, target, extensionElements) {
    ...
    const newExtensionElements = source.extensionElements.values.filter(value => extensionElements.some(extensionElement => ModelUtil.is(value, extensionElement)));
...
function createCustomEntry$1(id, element, property) {
  ...
  if (type === 'Number') {
    return {
      ...
      isEdited: propertiesPanel.isNumberFieldEntryEdited,
      property
    };
  }
  if (type === 'Boolean') {
