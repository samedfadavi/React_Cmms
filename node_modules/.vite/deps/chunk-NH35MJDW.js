...
async function E(e, t, a2, s2 = {}) {
  ...
  try {
    const { results: i2, edits: o2 } = await S(e, t, a2, s2), l2 = (e2) => e2.filter((e3) => !e3.error).map(a), d2 = { edits: o2, addedFeatures: l2(i2.addFeatureResults), updatedFeatures: l2(i2.updateFeatureResults), deletedFeatures: l2(i2.deleteFeatureResults), addedAttachments: l2(i2.addAttachmentResults), updatedAttachments: l2(i2.updateAttachmentResults), deletedAttachments: l2(i2.deleteAttachmentResults), exceededTransferLimit: false, historicMoment: i2.editMoment ? new Date(i2.editMoment) : null, globalIdToObjectId: s2.globalIdToObjectId };
    return ((_a = i2.editedFeatureResults) == null ? void 0 : _a.length) && (d2.editedFeatures = i2.editedFeatureResults), n2.resolve(d2), i2;
  } catch (d2) {
    throw n2.reject(d2), d2;
  }
}
async function S(e, t, r2, s2) {
  var _a, _b, _c, _d, _e, _f;
  if (await e.load(), !F(t)) throw new s(`${e.type}-layer:no-editing-support`, "Layer source does not support applyEdits capability", { layer: e });
  if (!B(e)) throw new s(`${e.type}-layer:editing-disabled`, "Editing is disabled for layer", { layer: e });
  const { edits: i2, options: n2 } = await R2(e, r2, s2);
  return ((_a = i2.addFeatures) == null ? void 0 : _a.length) || ((_b = i2.updateFeatures) == null ? void 0 : _b.length) || ((_c = i2.deleteFeatures) == null ? void 0 : _c.length) || ((_d = i2.addAttachments) == null ? void 0 : _d.length) || ((_e = i2.updateAttachments) == null ? void 0 : _e.length) || ((_f = i2.deleteAttachments) == null ? void 0 : _f.length) ? { edits: i2, results: await t.applyEdits(i2, n2) } : { edits: i2, results: { addFeatureResults: [], updateFeatureResults: [], deleteFeatureResults: [], addAttachmentResults: [], updateAttachmentResults: [], deleteAttachmentResults: [] } };
}
async function R2(e, t, r2) {
  ...
  if (M(t, i2, r2, !!n2, !!o, `${e.type}-layer`), !i2.data.isVersioned && (r2 == null ? void 0 : r2.gdbVersion)) throw new s(`${e.type}-layer:invalid-parameter`, "'gdbVersion' is applicable only if the layer supports versioned data. See: 'capabilities.data.isVersioned'");
  ...
  const u2 = V2(t, i2, `${e.type}-layer`), p2 = (r2 == null ? void 0 : r2.globalIdUsed) || l, c2 = e.fields.filter((e2) => "big-integer" === e2.type || "oid" === e2.type && (e2.length || 0) >= 8);
  if (p2) {
  ...
  u2.addFeatures.forEach((t2) => U(t2, e, p2, c2)), u2.updateFeatures.forEach((t2) => L2(t2, e, p2, c2)), u2.deleteFeatures.forEach((t2) => k(t2, e, p2, c2)), u2.addAttachments.forEach((t2) => B2(t2, e)), u2.updateAttachments.forEach((t2) => B2(t2, e)), l && await z(u2, e);
  return { edits: await D(u2), options: d2 };
}
function j(e, t, r2, s2) {
  ...
  if (s2.length && "attributes" in e) for (const i2 of s2) {
    ...
    if (void 0 !== r3 && !Te(i2, r3)) throw new s(`${t.type}-layer:invalid-parameter`, `Big-integer field '${i2.name}' of the feature must be less than ${Number.MAX_SAFE_INTEGER}`, { feature: e });
  }
...
function O2(e, t) {
  ...
  if ("geometry" in e && "mesh" === ((_a = e.geometry) == null ? void 0 : _a.type) && null != t.infoFor3D && null != t.spatialReference) {
    const { geometry: r2 } = e, { spatialReference: s2, vertexSpace: i2 } = r2, n2 = t.spatialReference, o = "local" === i2.type, l = A(n2), h = G(n2, s2), m = h || C(n2) && (C(s2) || w(s2));
...
async function D(e) {
  const t = e.addFeatures ?? [], a2 = e.updateFeatures ?? [], r2 = t.concat(a2).map((e2) => e2.geometry), s2 = await R(r2), i2 = t.length, n2 = a2.length;
...
function M(e, t, r2, s2, i2, n2) {
  ...
  if (!(r2 == null ? void 0 : r2.globalIdUsed) && i2) throw new s(`${n2}:invalid-parameter`, "When 'addAttachments', 'updateAttachments' or 'deleteAttachments' is specified, globalIdUsed should be set to true");
}
function x(t) {
  const a2 = new d();
...
async function z(e, t) {
  ...
  const { infoFor3D: r2 } = t, s2 = u("model/gltf-binary", r2.supportedFormats) ?? i("glb", r2.supportedFormats);
...
function W(e, t, r2, s2) {
  if (!F(t)) throw new s(`${e.type}-layer:no-editing-support`, "Layer source does not support applyEdits capability", { layer: e });
...
//# sourceMappingURL=chunk-NH35MJDW.js.map
