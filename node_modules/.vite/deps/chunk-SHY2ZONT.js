...
var o = class {
  constructor(r3, o3) {
    ...
    for (const t3 in r3) {
      const o4 = r3[t3], s4 = new s(o4, void 0, void 0, 2, 2);
  ...
  destroy() {
    ...
    for (const e3 in this._properties) {
      const t3 = this._properties[e3];
      for (const e4 of t3.acquired) O(e4) || t3.pool.release(e4);
      t3.pool.destroy(), t3.pool = null, t3.acquired = null;
    }
    this._properties = null, this._owner = null;
  }
  get(e3) {
    const t3 = this._owner._get(e3), r3 = this._properties[e3];
    let o3 = r3.pool.acquire();
    for (r3.acquired.push(o3); o3 === t3; ) r3.acquired.push(o3), o3 = r3.pool.acquire();
    return o3;
  }
  _release() {
    for (const e3 in this._properties) {
      const t3 = this._properties[e3];
      let o3 = 0;
      for (const e4 of t3.acquired) O(e4) ? t3.acquired[o3++] = e4 : t3.pool.release(e4);
...
var t2 = class {
  ...
  registerIncoming(t3, i3, s4) {
    ...
    }, g2 = new n2(a2, s4, { onPause: r3, onRemove: r3, onResume: (e3) => {
      const t4 = this._incoming[e3.match.eventType];
  ...
  registerOutgoing(e3) {
    ...
    const t3 = new i(e3, { onEmit: (e4, t4, n3, i3) => {
      var _a;
      (_a = this._manager) == null ? void 0 : _a.emit(e4.eventType, t4, n3, i3);
    }, onRemove: (e4) => {
      var _a;
  ...
  _handleEvent(e3) {
    ...
    if (t3) {
      for (const n3 of t3) if (n3.match.matches(e3) && ((_a = n3.callback) == null ? void 0 : _a.call(n3, e3), e3.shouldStopPropagation())) break;
    }
  }
};
var n2 = class {
...
var s3 = class extends t2 {
  ...
  _update(t3) {
    const s4 = "touch" === t3.native.pointerType ? "touch" : "mouse", { x: e3, y: i3 } = t3;
...
var d = class extends S {
  constructor(e3) {
    super(e3), this._pointerCaptures = /* @__PURE__ */ new Map(), this._nameToGroup = {}, this._handlers = [], this._handlersPriority = [], this._currentPropagation = null, this._updateDependenciesAfterPropagation = false, this._sourceEvents = /* @__PURE__ */ new Set(), this._keyModifiers = /* @__PURE__ */ new Set(), this._activeKeyModifiers = /* @__PURE__ */ new Set(), this._stoppedPropagationEventIds = /* @__PURE__ */ new Set(), this.primaryKey = t, this._latestPointerType = "mouse", this._propertiesPool = new o({ latestPointerLocation: P }, this), this.latestPointerLocation = null, this._paused = false, this.test = { timestamp: void 0, hasCurrentPropagation: () => !!this._currentPropagation };
  }
  initialize() {
  ...
  installHandlers(e3, t3, i3 = _.INTERNAL) {
    ...
    const r3 = { name: e3, handlers: t3.map((e4) => ({ handler: e4, active: true, removed: false, priorityIndex: 0, groupPriority: i3, eventCallback: null, uninstallCallback: null })) };
    ...
    for (let n3 = r3.handlers.length - 1; n3 >= 0; n3--) {
      ...
      this._handlers.push(e4), e4.handler.onInstall({ updateDependencies: () => {
        ...
        this._emitInputEvent(e4.priorityIndex + 1, t4, i4, r4, s4, n4);
  ...
  _installRecognizers() {
    this._latestPointerHandler = new s3((e3, t3, i3) => this._setLatestPointer(e3, t3, i3)), this._multiTouchHandler = new i2(), this.installHandlers("input-manager-logic", [this._latestPointerHandler, this._multiTouchHandler], _.ALWAYS), this.recognizers.length > 0 && this.installHandlers("default", this.recognizers, _.INTERNAL);
  }
  _setPointerCapture(e3, t3, i3, r3) {
    const n3 = e3.name + "-" + t3.priorityIndex, s4 = this._pointerCaptures.get(i3.pointerId) || /* @__PURE__ */ new Set();
    this._pointerCaptures.set(i3.pointerId, s4), r3 ? (s4.add(n3), 1 === s4.size && this.eventSource && this.eventSource.setPointerCapture(i3, true)) : s4.has(n3) && (s4.delete(n3), 0 === s4.size && (this._pointerCaptures.delete(i3.pointerId), this.eventSource && this.eventSource.setPointerCapture(i3, false)));
  }
  ...
  _emitInputEvent(e3, t3, i3, r3, n3, s4) {
    const o3 = void 0 !== r3 ? r3 : this._currentPropagation ? this._currentPropagation.timestamp : performance.now(), a2 = void 0 !== n3 && n3, h = { event: new u(t3, i3, o3, s4 || this._activeKeyModifiers, a2), priorityIndex: e3 };
    this._currentPropagation ? this._currentPropagation.events.push(h) : this._doNewPropagation(h);
  }
  _doNewPropagation(e3) {
    this._currentPropagation = { events: new s2(), currentHandler: null, needsHandlerGarbageCollect: false, timestamp: e3.event.timestamp }, this._currentPropagation.events.push(e3), this._continuePropagation();
  }
  _continuePropagation() {
    ...
    if (e3) {
      for (; e3.events.length > 0; ) {
        const { event: t3, priorityIndex: i3 } = e3.events.pop(), r3 = (_a = t3.data) == null ? void 0 : _a.eventId;
        if (!(null != r3 && this._stoppedPropagationEventIds.has(r3))) for (e3.currentHandler = this._handlersPriority[i3]; e3.currentHandler; ) {
          if (e3.currentHandler.removed) e3.needsHandlerGarbageCollect = true;
            ...
            if (e3.currentHandler.active && !t3.shouldStopPropagation() && ((_c = (_b = e3.currentHandler).eventCallback) == null ? void 0 : _c.call(_b, t3)), t3.shouldStopPropagation()) {
              null != r3 && this._stoppedPropagationEventIds.add(r3);
              break;
            }
            if (t3.shouldPausePropagation(() => this._continuePropagation())) return void this._pausePropagation({ event: t3, priorityIndex: e3.currentHandler.priorityIndex + 1 });
          }
...
//# sourceMappingURL=chunk-SHY2ZONT.js.map
