...
var z = class extends N {
  ...
  _writePoint(e4, t3, i5, s6) {
    ...
    o6 || (o6 = { offsetX: 0, offsetY: 0, size: 0 });
    const n6 = r4.boundsT, a4 = a2(this.evaluatedMeshParams.horizontalAlignment), l4 = c(this.evaluatedMeshParams.verticalAlignment), h4 = ((_a = this.evaluatedMeshParams.scaleInfo) == null ? void 0 : _a.maxScale) ?? 0, c6 = ((_b = this.evaluatedMeshParams.scaleInfo) == null ? void 0 : _b.minScale) ?? 0, m4 = f(s6.getDisplayId());
  ...
  _writeLines(e4, t3) {
    const { scaleInfo: i5, verticalAlignment: s6 } = this.evaluatedMeshParams, r4 = this.evaluatedMeshParams.repeatLabelDistance || 128, o6 = this._getShaping("middle");
    if (!o6) return;
    const n6 = (e5, t4, i6, s7) => this._placeSubdivGlyphs(e5, t4, i6, s7), a4 = (o6.bounds.width + r4) / (1 << L);
    this._current = { out: e4, id: t3.getDisplayId(), shaping: o6, zoomRange: m(i5, this.getTileInfo()), referenceBounds: this._getPointReferenceBounds() || { offsetX: 0, offsetY: 0, size: 0 }, offsetDirection: null }, this._verticalPlacement = "bottom" === s6 ? "above" : "top" === s6 ? "below" : null, this._verticalPlacement ? this._writeAboveAndBelowAlong(t3, n6, a4) : this._writeCenterAlong(t3, n6, a4);
  }
  _writeAboveAndBelowAlong(e4, i5, s6) {
    const { repeatLabel: r4, fontSize: o6 } = this.evaluatedMeshParams, { shaping: n6 } = this._current, a4 = u(o6), l4 = e4.readGeometryForDisplay();
    if (!l4) return;
    const h4 = new e();
  ...
  _writeCenterAlong(e4, t3, i5) {
    const { repeatLabel: s6 } = this.evaluatedMeshParams, { shaping: r4 } = this._current, o6 = l2(e4.readLegacyGeometryForDisplay().paths, r4.bounds.width);
    for (const n6 of o6) d(n6, i5, t3, !!s6);
  }
  _placeSubdivGlyphs(e4, t3, i5, s6) {
    const { allowOverrun: r4, labelPosition: o6, repeatLabelDistance: n6 } = this.evaluatedMeshParams, a4 = this._current.zoomRange[0], l4 = A(t3), h4 = this._current.shaping.bounds.width / (1 << L), c6 = Math.sqrt(n6 || I) / (1 << L), m4 = Math.min(i5, s6 - i5), d2 = this._current.shaping.isMultiline ? S : Math.log2(m4 / (c6 + h4 / 2)), u4 = 0 === t3 ? d2 : Math.min(l4, d2), f3 = Math.max(a4, this._zoomLevel + L - u4), g3 = this._zoomLevel - f3, p2 = this._current.shaping.bounds.width / 2 * 2 ** g3;
  ...
  _placeStraight(e4, t3) {
    var _a, _b;
    const { out: i5, id: s6, shaping: r4, referenceBounds: o6 } = this._current, { x: n6, y: a4 } = e4, l4 = f(s6), h4 = ((_a = this.evaluatedMeshParams.scaleInfo) == null ? void 0 : _a.maxScale) ?? 0, c6 = ((_b = this.evaluatedMeshParams.scaleInfo) == null ? void 0 : _b.minScale) ?? 0;
    ...
    this._writeGlyphs(i5, s6, n6, a4, r4, 0, o6, { clipAngle: m4, mapAligned: true, isLineLabel: true, minZoom: t3 }), this._writeGlyphs(i5, s6, n6, a4, r4, 0, o6, { clipAngle: d2, mapAligned: true, isLineLabel: true, minZoom: t3 }), i5.metricEnd();
  }
  _placeCurved(e4, t3, i5) {
    var _a, _b;
    const { out: s6, id: r4 } = this._current, o6 = e4.clone(), n6 = e4.angle * (180 / Math.PI) % 360, a4 = (e4.angle * (180 / Math.PI) + 180) % 360, l4 = f(r4), h4 = ((_a = this.evaluatedMeshParams.scaleInfo) == null ? void 0 : _a.maxScale) ?? 0, c6 = ((_b = this.evaluatedMeshParams.scaleInfo) == null ? void 0 : _b.minScale) ?? 0;
  ...
  _placeStraightAlong(e4, o6) {
    var _a, _b;
    const { out: n6, id: a4, shaping: l4, zoomRange: h4, referenceBounds: m4 } = this._current, { boxBorderLineColor: d2, boxBackgroundColor: u4 } = this.evaluatedMeshParams, f3 = e4.clone(), g3 = e4.angle * (180 / Math.PI) % 360, p2 = (e4.angle * (180 / Math.PI) + 180) % 360;
    if (l4.glyphs.length > 0 && !(!d2 && !u4)) {
      const d3 = Math.max(o6, h4[0], 0), u5 = Math.min(S, h4[1]), f4 = M(n3(), -e4.angle), _2 = { minZoom: d3, maxZoom: u5, clipAngle: g3, mapAligned: true, isLineLabel: true }, x2 = u(this.evaluatedMeshParams.offsetX), y = u(this.evaluatedMeshParams.offsetY);
    ...
    const x = f(a4), v3 = ((_a = this.evaluatedMeshParams.scaleInfo) == null ? void 0 : _a.maxScale) ?? 0, M2 = ((_b = this.evaluatedMeshParams.scaleInfo) == null ? void 0 : _b.minScale) ?? 0;
  ...
  _placeFirst(e4, t3, i5, s6, r4 = false) {
    const { out: o6, id: n6, shaping: a4, zoomRange: l4, referenceBounds: h4 } = this._current, c6 = a4.glyphs;
    for (const m4 of c6) {
  ...
  _placeOnSegment(e4, t3, i5, s6, r4, o6, n6) {
    const { out: a4, id: l4, shaping: h4, referenceBounds: c6 } = this._current, m4 = h4.glyphs, d2 = e4.dx / e4.length, u4 = e4.dy / e4.length, f3 = { x: e4.x + i5 * -r4 * d2, y: e4.y + i5 * -r4 * u4 };
    for (const g3 of m4) {
  ...
  _writeLineGlyph(e4, t3, i5, s6, r4, o6, n6, a4, l4) {
    ...
    const { texcoords: u4, offsets: f3 } = o6, g3 = this._textMeshTransformProps.fontSize;
    this._writeQuad(e4, t3, i5, s6, { texcoords: u4, offsets: f3, fontSize: g3, color: g(this.evaluatedMeshParams.color), isBackground: false, referenceBounds: a4, minZoom: Math.max(this._current.zoomRange[0], o6.minZoom), maxZoom: Math.min(this._current.zoomRange[1], o6.maxZoom), clipAngle: n6, mapAligned: l4, isLineLabel: true }), e4.recordEnd();
  }
  _isVisible(e4, t3) {
...
function D(e4, t3, i5) {
  const { coords: s6, lengths: r4 } = t3, c6 = n2(), d2 = n2(), u4 = n2(), f3 = n2(), g3 = n2(), p2 = n2(), _2 = 2;
  let x = 0;
...
async function c5(e4, r4, t3 = false) {
  const { defaultValue: s6, valueExpressionInfo: a4, value: o6 } = r4;
  if (a4) {
    const { expression: o7 } = a4, i5 = await e4.createComputedField({ expression: o7 }, t3);
    return i5 ? { ...r4, computed: i5, defaultValue: s6 } : null;
  }
  return { ...r4, computed: new r3(o6), defaultValue: s6 };
}
async function m3(e4, r4) {
  const { defaultValue: t3, valueExpressionInfo: s6 } = r4, { expression: a4 } = s6, o6 = await e4.createComputedField({ expression: a4 });
  return o6 ? { ...r4, computed: o6, defaultValue: t3 } : null;
}
...
var l3 = class _l {
      ...
      if (null != h4 && "object" == typeof h4) if (Array.isArray(h4)) {
        ...
        case "cim-effect-infos":
          if (h4.effectInfos.some((e4) => e4.overrides.length)) {
            i5.set(l4, { effects: await Promise.all(h4.effectInfos.map(async (t4) => {
              const s7 = t4.overrides.map((e4) => c5(r4, e4));
              return { effect: t4.effect, compiledOverrides: (await Promise.all(s7)).filter(O) };
            })) });
            break;
          }
        ...
        case "cim-marker-placement-param":
          h4.overrides.length && n6.set(l4, { placementInfo: h4, compiledOverrides: (await Promise.all(h4.overrides.map((e4) => c5(r4, e4)))).filter(O) }), a4[l4] = h4.placement;
          break;
        case "text-rasterization-param": {
          if (h4.overrides.length) {
            ...
            u4.set(l4, { compiledOverrides: (await Promise.all(t4)).filter(O), rasterizationParam: h4, objectIdToResourceId: /* @__PURE__ */ new Map() });
            continue;
          }
          const s7 = { type: "cim-rasterization-info", resource: h4.resource };
          a4[l4] = await t3.fetchResourceImmediate(s7) ?? null;
          break;
        }
        case "sprite-rasterization-param": {
          ...
          if ("animated" === h4.resource.type) {
            u4.set(l4, { compiledOverrides: [], rasterizationParam: h4, objectIdToResourceId: /* @__PURE__ */ new Map() });
            continue;
          }
          const s7 = { type: "cim-rasterization-info", resource: h4.resource };
          a4[l4] = await t3.fetchResourceImmediate(s7) ?? null;
        ...
        case "cim-marker-transform-param": {
          ...
          } else a4[l4] = { type: "cim-marker-transform-info", infos: e4 };
          break;
        }
        default:
          a4[l4] = h4;
  ...
  enqueueRequest(e4, s6, i5) {
    for (const m4 of this._dynamicAsyncProperties.values()) {
      ...
      for (const { primitiveName: e5, propertyName: t3, computed: a4, defaultValue: l4, valueExpressionInfo: u4 } of m4.compiledOverrides) try {
        const r4 = "animated" === m4.rasterizationParam.resource.type ? p2.primitiveName : e5;
        o3(p2, r4, t3, a4, s6, i5, l4);
      } catch (c6) {
        n5().errorOnce(new s("invalid-arcade-expression", `Encountered an error when evaluating the arcade expression '${u4 == null ? void 0 : u4.expression}' (primitive: '${e5}', property: '${t3}')`, c6));
      }
      const f3 = e4.enqueueRequest({ type: "cim-rasterization-info", resource: p2 });
  ...
  evaluateMeshParams(e4, r4, t3) {
    ...
    for (const [s6, a4] of this._dynamicPlacementProperties.entries()) for (const { computed: e5, defaultValue: o6, propertyName: i5 } of a4.compiledOverrides) {
    ...
    for (const [s6, a4] of this._dynamicEffectProperties.entries()) for (const e5 of a4.effects) {
      for (const { computed: s7, defaultValue: a5, propertyName: o6 } of e5.compiledOverrides) {
...
var W = class {
  async createMeshWriter(e4, r4, t3, i5) {
    const s6 = this._getMeshWriter(i5.techniqueType), l4 = await l3.create(e4, r4, i5.inputParams), a4 = new s6(i5.id, l4, i5.optionalAttributes, t3);
    return await a4.loadDependencies(), a4;
  }
  _getMeshWriter(c6) {
    switch (c6) {
      case e2.Fill:
...
//# sourceMappingURL=chunk-V5SM2NNT.js.map
