...
var e = { color: [128, 128, 128, 1], outlinecolor: [0, 0, 0, 1], backgroundcolor: [128, 128, 128, 1], borderlinecolor: [0, 0, 0, 1], tintcolor: [255, 255, 255, 1] };
...
function l(e2, t2) {
  ...
  let n = 0, o2 = -1;
  do {
    if (o2 = e2.indexOf("[", n), o2 >= n) {
      if (o2 > n) {
        const t3 = e2.substr(n, o2 - n);
        r.push([t3, null, a(t3)]);
      }
      if (n = o2 + 1, o2 = e2.indexOf("]", n), o2 >= n) {
        if (o2 > n) {
          const i2 = t2[e2.substr(n, o2 - n)];
          i2 && r.push([null, i2, false]);
        }
        n = o2 + 1;
      }
    }
  ...
  if (n < e2.length - 1) {
    const t3 = e2.substr(n);
    r.push([t3, null, a(t3)]);
  }
  return r;
}
function s(e2, t2, r, n) {
  let o2 = "", i2 = null;
  for (const a2 of t2) {
    const [t3, n2, l2] = a2;
...
function u(e2, t2, r, n, o2, i2, a2 = true) {
  const l2 = t2 / o2, s2 = r / i2, c2 = Math.ceil(l2 / 2), u2 = Math.ceil(s2 / 2);
  for (let f2 = 0; f2 < i2; f2++) for (let r2 = 0; r2 < o2; r2++) {
    const y2 = 4 * (r2 + (a2 ? i2 - f2 - 1 : f2) * o2);
    let M2 = 0, m2 = 0, C2 = 0, b2 = 0, I2 = 0, p2 = 0, h2 = 0;
    const d2 = (f2 + 0.5) * s2;
    for (let n2 = Math.floor(f2 * s2); n2 < (f2 + 1) * s2; n2++) {
      const o3 = Math.abs(d2 - (n2 + 0.5)) / u2, i3 = (r2 + 0.5) * l2, a3 = o3 * o3;
      for (let s3 = Math.floor(r2 * l2); s3 < (r2 + 1) * l2; s3++) {
        let r3 = Math.abs(i3 - (s3 + 0.5)) / c2;
        const o4 = Math.sqrt(a3 + r3 * r3);
        o4 >= -1 && o4 <= 1 && (M2 = 2 * o4 * o4 * o4 - 3 * o4 * o4 + 1, M2 > 0 && (r3 = 4 * (s3 + n2 * t2), h2 += M2 * e2[r3 + 3], C2 += M2, e2[r3 + 3] < 255 && (M2 = M2 * e2[r3 + 3] / 250), b2 += M2 * e2[r3], I2 += M2 * e2[r3 + 1], p2 += M2 * e2[r3 + 2], m2 += M2));
      }
    }
    n[y2] = b2 / m2, n[y2 + 1] = I2 / m2, n[y2 + 2] = p2 / m2, n[y2 + 3] = h2 / C2;
  }
}
function f(e2) {
...
function P(e2) {
  let t2 = "normal", r = "normal";
  if (e2) {
    const n = e2.toLowerCase();
    n.includes("italic") ? t2 = "italic" : n.includes("oblique") && (t2 = "oblique"), n.includes("bold") ? r = "bold" : n.includes("light") && (r = "lighter");
  }
  return { style: t2, weight: r };
}
function w(e2) {
...
function z(e2, t2, r, n, o2, i2) {
  const { infos: a2 } = i2;
  let l2 = 1, s2 = 0, c2 = 0, u2 = 0;
  for (const { offsetX: M2, offsetY: m2, rotation: C2, size: b2, frameHeight: I2, rotateClockWise: p2 } of a2) {
    c2 = U(M2, m2, s2, l2, c2), u2 = J(M2, m2, s2, l2, u2), I2 && (l2 *= b2 / I2), s2 = W(C2, p2, s2);
  }
  const f2 = U(n, o2, s2, l2, c2), y2 = J(n, o2, s2, l2, u2);
  return { size: e2 * l2, rotation: W(t2, r, s2), offsetX: f2, offsetY: y2 };
}
function E(e2) {
...
function O(r, n) {
  ...
  const o2 = t[r];
  if (!o2) throw new Error(`InternalError: default value for type ${r}.`);
  return o2[n];
}
function D(e2) {
...
function q(e2) {
  if (e2) switch (e2.type) {
    ...
    case "CIMPointSymbol": {
      let r = 0;
      if (e2.symbolLayers) {
        for (const n of e2.symbolLayers) if (n) switch (n.type) {
          case "CIMCharacterMarker":
          case "CIMPictureMarker":
          case "CIMVectorMarker":
          case "CIMObjectMarker3D":
          case "CIMglTFMarker3D": {
            const e3 = n.size ?? t.CIMVectorMarker.size;
            e3 > r && (r = e3);
            break;
          }
        }
    ...
    case "CIMPolygonSymbol": {
      ...
      if (e2.symbolLayers) {
        for (const n of e2.symbolLayers) if (n) switch (n.type) {
          ...
          case "CIMObjectMarker3D":
          case "CIMglTFMarker3D":
            if (n.markerPlacement && b(n.markerPlacement)) {
              const e3 = n.size ?? t.CIMVectorMarker.size;
              e3 > r && (r = e3);
            }
            break;
...
function R(e2) {
  return e2.map((e3) => ({ ...e3, propertyName: v(e3.propertyName) }));
}
function H(e2) {
  const t2 = {};
  for (const r in e2) {
...
function J(e2, t2, r, n, o2) {
  const i2 = r * Math.PI / 180;
  if (i2) {
    const r2 = Math.cos(i2);
    return (Math.sin(i2) * e2 + r2 * t2) * n + o2;
  }
  return t2 * n + o2;
}
function U(e2, t2, r, n, o2) {
  const i2 = r * Math.PI / 180;
  if (i2) {
    return (Math.cos(i2) * e2 - Math.sin(i2) * t2) * n + o2;
  }
  return e2 * n + o2;
}
function W(e2, t2, r) {
...
//# sourceMappingURL=chunk-D6KE2FST.js.map
