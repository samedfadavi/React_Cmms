...
function y(e2, s3, n4) {
  switch (e2.toLowerCase().trim()) {
    ...
    case "log":
    case "abs":
      if (1 !== s3.length) throw new n(t.InvalidFunctionParameters, { function: e2.toLowerCase().trim() });
      return `${e2.toUpperCase().trim()}(${s3[0]})`;
    case "ceiling":
    case "ceil":
      if (1 !== s3.length) throw new n(t.InvalidFunctionParameters, { function: "ceiling" });
      switch (n4) {
        case l.Standardised:
    ...
    case "power":
    case "nullif":
      if (2 !== s3.length) throw new n(t.InvalidFunctionParameters, { function: e2.toLowerCase().trim() });
    ...
    case "truncate":
      if (s3.length < 1 || s3.length > 2) throw new n(t.InvalidFunctionParameters, { function: "truncate" });
      return n4 === l.SqlServer ? "ROUND(" + s3[0] + (1 === s3.length ? "0" : "," + s3[1]) + ",1)" : "TRUNCATE(" + s3[0] + (1 === s3.length ? ")" : "," + s3[1] + ")");
    case "char_length":
    case "len":
      if (1 !== s3.length) throw new n(t.InvalidFunctionParameters, { function: "char_length" });
      switch (n4) {
        case l.SqlServer:
          return "LEN(" + s3[0] + ")";
        case l.Oracle:
    ...
    case "coalesce":
    case "concat": {
      if (s3.length < 1) throw new n(t.InvalidFunctionParameters, { function: e2.toLowerCase() });
    ...
    case "lower":
    case "lcase":
      if (1 !== s3.length) throw new n(t.InvalidFunctionParameters, { function: "lower" });
      return "LOWER(" + s3[0] + ")";
    case "upper":
    case "ucase":
      if (1 !== s3.length) throw new n(t.InvalidFunctionParameters, { function: "upper" });
      return "UPPER(" + s3[0] + ")";
    case "substring": {
      let e3 = "";
      switch (n4) {
...
function I(e2, a2, n4, l2) {
  ...
  switch (a2.type) {
    ...
    case "binary-expression":
      switch (a2.operator) {
        ...
        case "||":
          return "string";
        default:
          throw new n(t.UnsupportedOperator, { operator: a2.operator });
      }
    case "null":
      return "";
    case "boolean":
    ...
    case "function":
      switch (a2.name.toLowerCase()) {
        case "cast":
          switch (((_b = (_a = a2.args) == null ? void 0 : _a.value[1]) == null ? void 0 : _b.value.type) ?? "") {
            case "integer":
            case "smallint":
              return "integer";
            case "real":
            ...
            case "date":
            case "timestamp":
              return true === ((_e = (_d = (_c = a2.args) == null ? void 0 : _c.value[1]) == null ? void 0 : _d.value) == null ? void 0 : _e.withtimezone) ? "timestamp-offset" : "date";
            case "time":
              return "time-only";
            case "varchar":
              return "string";
  ...
  throw new n(t.UnsupportedSyntax, { node: a2.type });
}
...
async function v3(t3, n4, e2) {
  const r3 = await O5(t3, n4, e2, true);
...
async function M2(t3, n4, e2) {
  ...
  const a2 = await O5(t3, n4, e2, true);
...
async function O5(e2, r3, a2, c2 = false) {
  const s3 = e2.iterator(a2), u4 = [], f6 = { ticker: 0 };
  ...
  for (; null !== h4; ) {
    ...
    null === e3 ? false === c2 && (u4[u4.length] = e3) : u4[u4.length] = e3 instanceof n2 || e3 instanceof n3 ? e3.toNumber() : e3 instanceof r2 ? e3.toMilliseconds() : e3, h4 = await s3.next();
  }
  return u4;
}
async function S4(e2, r3, a2 = 1e3, c2 = null) {
  const s3 = e2.iterator(c2), u4 = [], f6 = {}, h4 = { ticker: 0 };
  let m5 = await s3.next();
  for (; null !== m5; ) {
    if (h4.ticker++, c2 == null ? void 0 : c2.aborted) throw new s(r.Cancelled);
...
var x3 = class _x {
  ...
  async _getAggregatePagesDataSourceDefinition(e2, n4, i, a2, r3, l2, u4) {
  ...
  async _getFeature(e2, n4, i) {
    ...
    if (await this._getFeatures(e2, n4, this._maxProcessingRate(), i), this._checkCancelled(i), void 0 !== this._featureFromCache(n4)) return this._featureFromCache(n4);
  ...
  async _refineSetBlock(e2, t3, s3) {
    if (true === this._checkIfNeedToExpandCandidatePage(e2, this._maxQueryRate())) return await this._expandPagedSet(e2, this._maxQueryRate(), 0, 0, s3), this._refineSetBlock(e2, t3, s3);
    ...
    if (await this._refineIfParentKnown(e2, t3 - i, s3), this._checkCancelled(s3), this._refineKnowns(e2, t3 - i), i = n4 - e2._candidates.length, i < t3 && e2._candidates.length > 0) {
      ...
      return await this._fetchAndRefineFeatures(a2, a2.length > n5 ? n5 : e2._candidates.length, s3), this._checkCancelled(s3), this._refineKnowns(e2, t3 - i), e2;
    }
    return e2;
  }
  ...
  _refineKnowns(e2, t3) {
    ...
    for (let a2 = 0; a2 < e2._candidates.length && "GETPAGES" !== e2._candidates[a2]; a2++) {
      ...
      if (h4 === s2.InFeatureSet) true === r3 ? e2._known.includes(u4) || (e2._known.push(u4), s3 += 1) : (e2._known.push(e2._candidates[a2]), s3 += 1), null === n4 ? n4 = { start: a2, end: a2 } : n4.end === a2 - 1 ? n4.end = a2 : (i.push(n4), n4 = { start: a2, end: a2 });
      else if (h4 === s2.NotInFeatureSet) null === n4 ? n4 = { start: a2, end: a2 } : n4.end === a2 - 1 ? n4.end = a2 : (i.push(n4), n4 = { start: a2, end: a2 }), s3 += 1;
  ...
  async _expandPagedSet(e2, n4, i, a2, r3) {
  ...
  async _expandPagedSetFeatureSet(e2, t3, s3, n4, i) {
    ...
    const a2 = await this._getPage(e2, n4, i);
  ...
  async _manualStat(e2, t3, s3, n4) {
    ...
    switch (e2.toLowerCase()) {
      ...
      case "variance":
        return i = await x2(this, t3, n4), { calculated: true, result: i };
      case "sum":
  ...
  async _stat(e2, t3, s3, n4, i, a2, r3) {
    const l2 = await this._parent._stat(e2, t3, s3, n4, i, a2, r3);
    return false === l2.calculated ? null === i && "" === s3 && null === n4 ? this._manualStat(e2, t3, a2, r3) : { calculated: false } : l2;
  }
  ...
  async count(e2) {
    return await this.load(), this.calculateStatistic("count", O.create("1", this.getFieldsIndex(), this.dateFieldsTimeZoneDefaultUTC), -1, this._defaultTracker(e2));
  }
  _defaultTracker(e2) {
...
var f5 = class _f extends x3 {
  ...
  async _getFilteredSet(e2, t3, i, r3, a2) {
    await this._ensureLoaded();
    const s3 = await this._parent._getFilteredSet("esriSpatialRelRelation" !== this._relation ? this._relation : this._relation + ":" + this._relationString, this._relationGeom, i, r3, a2);
    let l2;
  ...
  async _stat(e2, t3, i, r3, n4, a2, s3) {
    if ("" !== i) return { calculated: false };
    const l2 = await this._parent._stat(e2, t3, "esriSpatialRelRelation" !== this._relation ? this._relation : this._relation + ":" + this._relationString, this._relationGeom, n4, a2, s3);
    return false === l2.calculated ? null === n4 && "" === i && null === r3 ? this._manualStat(e2, t3, a2, s3) : { calculated: false } : l2;
  }
  ...
  async _getAggregatePagesDataSourceDefinition(e2, r3, n4, a2, s3, l2, o) {
    if (null === this._parent) throw new s(r.NeverReach);
...
//# sourceMappingURL=chunk-PQKQN45E.js.map
