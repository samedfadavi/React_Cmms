...
var o4 = (t6, e8, o5, n7) => {
  const a2 = t6 + t6.substring(t6.lastIndexOf("/")), p = e8 + e8.substring(e8.lastIndexOf("/")), f3 = s3(n7);
  return { attributes: o5, shaders: { vertexShader: f3 + n4(`${a2}.vert`), fragmentShader: f3 + n4(`${p}.frag`) } };
};
...
var e3 = class {
  constructor(e8) {
    this._objectType = e8, this._active = /* @__PURE__ */ new Map();
  }
  add(e8) {
    const t6 = new Error().stack.split("\n");
    t6.shift(), t6.shift(), this._active.set(e8, t6.join("\n"));
  }
  remove(e8) {
  ...
  resetLog() {
    ...
    this._active.forEach((t7, { usedMemory: s5 }) => {
      e8.has(t7) || e8.set(t7, /* @__PURE__ */ new Map());
      const r6 = e8.get(t7);
      r6.set(s5 ?? 0, r6.get(s5 ?? 0) ?? 1);
    }), this._active.clear();
    let t6 = 0;
    const s4 = new Array();
    return e8.forEach((e9, r6) => {
      let a2 = 0;
      e9.forEach((e10, t7) => a2 += e10 * t7), e9.set(-1, a2), t6 += a2, s4.push([r6, e9]);
    }), e8.clear(), s4.sort((e9, t7) => t7[1].get(-1) - e9[1].get(-1)), s4.reduce((e9, [t7, s5]) => {
      const r6 = Math.round(s5.get(-1) / 1024);
      s5.delete(-1);
      return e9 += `  ${r6}KB from ${Array.from(s5.values()).reduce((e10, t8) => e10 + t8, 0)} allocations at ${t7}
`;
...
var n5 = class {
  constructor() {
    for (this._current = new Array(), this._allocations = e4.RECORD_ALLOCATIONS ? new e3("WebGLObject") : null; this._current.length < V.COUNT; ) this._current.push(0);
  }
  increment(t6, r6, e8 = 1) {
    var _a;
    this._current[t6] += e8, (_a = this._allocations) == null ? void 0 : _a.add(r6);
  }
  decrement(t6, r6, e8 = 1) {
    var _a;
    this._current[t6] -= e8, (_a = this._allocations) == null ? void 0 : _a.remove(r6);
  }
  get current() {
    return this._current;
  }
  get total() {
    return this.current.reduce((t6, e8, n7) => t6 + (n7 < V.UNCOUNTED ? e8 : 0), 0);
  }
...
var d = class extends t4 {
  constructor(e8) {
    super(), this._rctx = e8, this._helperProgram = null, has("mac") && has("chrome") && (this._program = this._prepareProgram(), this._helperProgram = this._prepareHelperProgram());
  }
  dispose() {
    var _a;
  ...
  _test(e8) {
    ...
    r6.bindFramebuffer(P2), r6.setViewport(0, 0, 1, 1), r6.useProgram(this._helperProgram), r6.bindBuffer(w, A2.ELEMENT_ARRAY_BUFFER), r6.drawElements(E.POINTS, 1, C.UNSIGNED_BYTE, 0), r6.useProgram(e8), r6.bindVAO(null), r6.drawArrays(E.TRIANGLES, 0, 258);
...
var _2 = class extends t4 {
  ...
  _test(e8) {
    ...
    const n7 = this._rctx, t6 = n7.getBoundFramebufferObject(), { x: r6, y: o5, width: i2, height: s4 } = n7.getViewport(), c3 = this._runProgram(e8), a2 = this._runProgram(this._obfuscated);
    return n7.setViewport(r6, o5, i2, s4), n7.bindFramebuffer(t6), 0 !== c3 && (0 === a2 || c3 / a2 > 5);
  }
  _runProgram(n7) {
    const d2 = this._rctx;
    d2.resetState();
    ...
    const A4 = new E2(d2, _4), b2 = c.createVertex(d2, F.STATIC_DRAW, new Uint16Array([0, 0, 1, 0, 0, 1, 1, 1])), g = new o3(d2, /* @__PURE__ */ new Map([["position", 0]]), { geometry: [new t("position", 2, C.UNSIGNED_SHORT, 0, 4)] }, { geometry: b2 }), B = r2(5633261287538229e-9, 2626832878767164e-9, 1.4349880495278358e6), w = r2(563327146742708e-8, 2.6268736381334523e6, 1434963231608387e-9), F2 = new Float32Array(6);
    t2(B, F2, 3);
...
function A3(e8, n7) {
  ...
  return e8.programCache.acquire(t6, r6, /* @__PURE__ */ new Map([["position", 0]]));
}
...
var b = class extends t4 {
  constructor(e8) {
    var _a, _b2;
    if (super(), this._rctx = e8, !e8.gl) return;
    if (!(((_a = e8.capabilities.colorBufferFloat) == null ? void 0 : _a.textureFloat) && ((_b2 = e8.capabilities.colorBufferFloat) == null ? void 0 : _b2.floatBlend))) return;
    const r6 = "\n    precision highp float;\n    attribute vec2 a_pos;\n\n    void main() {\n      gl_Position = vec4(a_pos * 2.0 - 1.0, 0.0, 1.0);\n    }\n    ", t6 = "\n     precision highp float;\n\n     void main() {\n      gl_FragColor = vec4(0.5, 0.5, 0.5, 0.5);\n     }\n    ";
    this._program = e8.programCache.acquire(r6, t6, /* @__PURE__ */ new Map([["a_pos", 0]]));
  }
  _test(g) {
    ...
    const A4 = new E2(b2, d2), w = c.createVertex(b2, F.STATIC_DRAW, new Uint16Array([0, 0, 1, 0, 0, 1, 1, 1])), T2 = new o3(b2, /* @__PURE__ */ new Map([["a_pos", 0]]), { geometry: [new t("a_pos", 2, C.UNSIGNED_SHORT, 0, 4)] }, { geometry: w });
    b2.gl.getError(), b2.useProgram(g);
    const E4 = b2.getBoundFramebufferObject(), { x: j, y: O2, width: x, height: h2 } = b2.getViewport();
    b2.bindFramebuffer(A4), b2.setViewport(0, 0, 1, 1), b2.bindVAO(T2), b2.drawArrays(E.TRIANGLE_STRIP, 0, 4);
    const F2 = S2({ blending: s(R.ZERO, R.ONE_MINUS_SRC_ALPHA) });
...
function t5(_4, E4) {
  ...
  return R3 || t6 || S4 ? { textureFloat: !!t6, textureHalfFloat: !!R3, floatBlend: !!S4, R16F: _4.R16F, RG16F: _4.RG16F, RGBA16F: _4.RGBA16F, R32F: _4.R32F, RG32F: _4.RG32F, RGBA32F: _4.RGBA32F, R11F_G11F_B10F: _4.R11F_G11F_B10F, RGB16F: _4.RGB16F } : null;
}
function S3(_4, E4, R3, t6, S4) {
  if (t6) return true;
  if (E4[R3]) return false;
  for (const e8 of S4) if (_4.getExtension(e8)) return true;
  return false;
}
function e6(_4, E4) {
...
function e7(e8, t6) {
  const n7 = t6.loseContext && e8.getExtension("WEBGL_lose_context");
...
var u2 = class {
  constructor(t6, e8) {
    this._gl = t6, this._compressedTextureETC = null, this._compressedTextureS3TC = null, this._textureFilterAnisotropic = null, this._colorBufferFloat = null, this._loseContext = null, this._textureNorm16 = null, this._textureFloatLinear = null, this._disabledExtensions = e8.disabledExtensions || {}, this._debugWebGLExtensions = e8.debugWebGLExtensions || {};
  }
  ...
  get colorBufferFloat() {
    return this._colorBufferFloat ?? (this._colorBufferFloat = t5(this._gl, this._disabledExtensions)), this._colorBufferFloat;
  }
  get loseContext() {
    return this._loseContext ?? (this._loseContext = e7(this._gl, this._debugWebGLExtensions)), this._loseContext;
  }
  get textureNorm16() {
    return this._textureNorm16 ?? (this._textureNorm16 = e6(this._gl, this._disabledExtensions)), this._textureNorm16;
  }
  get textureFloatLinear() {
    return this._textureFloatLinear ?? (this._textureFloatLinear = S3(this._gl, this._disabledExtensions, "textureFloatLinear", false, ["OES_texture_float_linear"])), this._textureFloatLinear;
  }
  enable(t6) {
    return this[t6];
  }
...
var y = class {
  constructor(t6, e8) {
    this.gl = t6, this.instanceCounter = new n5(), this.programCache = new s2(this), this._transformFeedbackRequestInfo = null, this._state = new h(), this._numOfDrawCalls = 0, this._numOfTriangles = 0, this._loadExtensions(), this.configure(e8);
  }
  configure(e8) {
    this._capabilities = new u2(this.gl, e8), this._parameters = new e5(this.gl, this._capabilities, e8), c2.TEXTURE_UNIT_FOR_UPDATES = this._parameters.maxTextureImageUnits - 1;
    const s4 = this.gl.getParameter(this.gl.VIEWPORT);
    this._state = new h(), this._state.viewport = { x: s4[0], y: s4[1], width: s4[2], height: s4[3] }, this._stateTracker = new G2({ setBlending: (t6) => {
      if (t6) {
  ...
  externalTextureUnitUpdate(t6, e8) {
    for (let s4 = 0; s4 < t6.length; ++s4) this._state.textureUnitMap[t6[s4]] = null;
  ...
  setBlendColor(t6, e8, s4, i2) {
    t6 === this._state.blendColor.r && e8 === this._state.blendColor.g && s4 === this._state.blendColor.b && i2 === this._state.blendColor.a || (this.gl.blendColor(t6, e8, s4, i2), this._state.blendColor.r = t6, this._state.blendColor.g = e8, this._state.blendColor.b = s4, this._state.blendColor.a = i2, this._stateTracker.invalidateBlending());
  }
  ...
  setBlendFunctionSeparate(t6, e8, s4, i2) {
    this._state.blendFunction.srcRGB === t6 && this._state.blendFunction.srcAlpha === s4 && this._state.blendFunction.dstRGB === e8 && this._state.blendFunction.dstAlpha === i2 || (this.gl.blendFuncSeparate(t6, e8, s4, i2), this._state.blendFunction.srcRGB = t6, this._state.blendFunction.srcAlpha = s4, this._state.blendFunction.dstRGB = e8, this._state.blendFunction.dstAlpha = i2, this._stateTracker.invalidateBlending());
  }
  setBlendEquation(t6) {
  ...
  setColorMask(t6, e8, s4, i2) {
    this._state.colorMask.r === t6 && this._state.colorMask.g === e8 && this._state.colorMask.b === s4 && this._state.colorMask.a === i2 || (this.gl.colorMask(t6, e8, s4, i2), this._state.colorMask.r = t6, this._state.colorMask.g = e8, this._state.colorMask.b = s4, this._state.colorMask.a = i2, this._stateTracker.invalidateColorWrite());
  }
  setClearColor(t6, e8, s4, i2) {
    this._state.clearColor.r === t6 && this._state.clearColor.g === e8 && this._state.clearColor.b === s4 && this._state.clearColor.a === i2 || (this.gl.clearColor(t6, e8, s4, i2), this._state.clearColor.r = t6, this._state.clearColor.g = e8, this._state.clearColor.b = s4, this._state.clearColor.a = i2);
  }
  ...
  setScissorRect(t6, e8, s4, i2) {
    this._state.scissorRect.x === t6 && this._state.scissorRect.y === e8 && this._state.scissorRect.width === s4 && this._state.scissorRect.height === i2 || (this.gl.scissor(t6, e8, s4, i2), this._state.scissorRect.x = t6, this._state.scissorRect.y = e8, this._state.scissorRect.width = s4, this._state.scissorRect.height = i2);
  }
  ...
  setStencilFunctionSeparate(t6, e8, s4, i2) {
    this._state.stencilFunction.face === t6 && this._state.stencilFunction.func === e8 && this._state.stencilFunction.ref === s4 && this._state.stencilFunction.mask === i2 || (this.gl.stencilFuncSeparate(t6, e8, s4, i2), this._state.stencilFunction.face = t6, this._state.stencilFunction.func = e8, this._state.stencilFunction.ref = s4, this._state.stencilFunction.mask = i2, this._stateTracker.invalidateStencilTest());
  }
  ...
  setStencilOpSeparate(t6, e8, s4, i2) {
    this._state.stencilOperation.face === t6 && this._state.stencilOperation.fail === e8 && this._state.stencilOperation.zFail === s4 && this._state.stencilOperation.zPass === i2 || (this.gl.stencilOpSeparate(t6, e8, s4, i2), this._state.stencilOperation.face = t6, this._state.stencilOperation.fail = e8, this._state.stencilOperation.zFail = s4, this._state.stencilOperation.zPass = i2, this._stateTracker.invalidateStencilTest());
  }
  ...
  setDrawBuffers(t6) {
    const { drawFramebuffer: e8 } = this._state, s4 = null === e8, i2 = s4 ? this._state.drawBuffers.defaultFramebuffer : this._state.drawBuffers.fbos.get(e8);
    if ((i2 == null ? void 0 : i2.length) !== t6.length || !i2.every((e9, s5) => e9 === t6[s5])) if (t6.length > this.parameters.maxDrawBuffers) console.error("Setting more active draw buffers than GL.MAX_DRAW_BUFFERS allows.");
    else {
      if (s4) {
      ...
      s4 || !t6.includes(f.BACK) ? (this.gl.drawBuffers(t6), s4 ? this._state.drawBuffers.defaultFramebuffer = t6 : this._state.drawBuffers.fbos.set(e8, t6), this._stateTracker.invalidateDrawBuffers()) : console.error("A framebuffer object can only use the constants GL.COLOR_ATTACHMENTi or GL.NONE as draw buffers.");
    }
  }
  clear(t6, e8 = 255) {
    var _a;
    if (t6) {
      if (t6 & _.COLOR_BUFFER_BIT) {
        const t7 = (_a = this._state.drawFramebuffer) == null ? void 0 : _a.colorAttachments;
        t7 && this.setDrawBuffers(t7), this.setColorMask(true, true, true, true);
      }
      t6 & _.DEPTH_BUFFER_BIT && this.setDepthWriteEnabled(true), t6 & _.STENCIL_BUFFER_BIT && this.setStencilWriteMask(e8), this.gl.clear(t6);
    }
  }
  clearFramebuffer(t6, e8 = false, s4 = false) {
    let i2 = 0;
    if (t6) {
      const e9 = 1e-13, s5 = Math.max(e9, t6[3]);
      this.setClearColor(t6[0], t6[1], t6[2], s5), i2 |= _.COLOR_BUFFER_BIT;
    }
    e8 && (i2 |= _.DEPTH_BUFFER_BIT), false === s4 ? s4 = 0 : (true === s4 && (s4 = 255), i2 |= _.STENCIL_BUFFER_BIT), i2 && this.clear(i2, s4);
  }
  drawArrays(t6, e8, s4) {
    var _a, _b;
    ...
    if (a()) {
      this._numOfDrawCalls++, this._numOfTriangles += D2(t6, s4);
      const e9 = this._state.textureUnitMap;
      for (let t7 = 0; t7 < e9.length; t7++) {
        const s5 = e9[t7];
        if (null != s5 && s5 === ((_b = this._state.drawFramebuffer) == null ? void 0 : _b.colorTexture)) throw new Error(`Detected readWrite. Texture already bound at index ${t7}`);
      }
    }
    this.gl.drawArrays(t6, e8, s4), u(this.gl);
  }
  drawArraysInstanced(t6, e8, s4, i2) {
    this.gl.drawArraysInstanced(t6, e8, s4, i2), u(this.gl);
  }
  drawElements(t6, e8, s4, a2) {
    if (this._transformFeedbackRequestInfo) throw new Error("Cannot called drawElements during a transform feedback request");
    if (a() && (this._numOfDrawCalls++, this._numOfTriangles += D2(t6, e8)), this.gl.drawElements(t6, e8, s4, a2), a()) {
      const i2 = o(this);
      if (i2) {
        const r6 = this.getBoundVAO(), n7 = r6 == null ? void 0 : r6.indexBuffer, l = r6 == null ? void 0 : r6.vertexBuffers, h2 = { indexBuffer: n7, vertexBuffers: l }, f3 = { mode: t6, count: e8, type: s4, offset: a2 }, o5 = (n7 == null ? void 0 : n7.size) ?? 0, u3 = a2 + e8, c3 = o5 < u3 ? `. Buffer is too small. Attempted to draw index ${u3} of ${o5}` : "";
        console.error(`drawElements: ${i2}${c3}`, { args: f3, vao: h2 });
      }
    }
  }
  logInfo() {
  ...
  setViewport(t6, e8, s4, i2) {
    s4 = Math.max(Math.round(s4), 1), i2 = Math.max(Math.round(i2), 1);
    const a2 = this._state.viewport;
    a2.x === t6 && a2.y === e8 && a2.width === s4 && a2.height === i2 || (a2.x = t6, a2.y = e8, a2.width = s4, a2.height = i2, this.gl.viewport(t6, e8, s4, i2));
  }
  setViewport4fv(t6) {
    this.setViewport(t6[0], t6[1], t6[2], t6[3]);
  }
  restoreViewport({ x: t6, y: e8, width: s4, height: i2 }) {
    this.setViewport(t6, e8, s4, i2);
  }
  getViewport() {
  ...
  bindTexture(t6, e8, s4 = false) {
    ...
    return null == (t6 == null ? void 0 : t6.glName) ? (null != i2 && (this.setActiveTexture(e8, s4), this.gl.bindTexture(i2.descriptor.target, null)), this._state.textureUnitMap[e8] = null, i2) : s4 || i2 !== t6 ? (this.setActiveTexture(e8, s4), this.gl.bindTexture(t6.descriptor.target, t6.glName), t6.applyChanges(), this._state.textureUnitMap[e8] = t6, i2) : (t6.isDirty && (this.setActiveTexture(e8, s4), t6.applyChanges()), i2);
  }
  unbindTexture(t6) {
    if (null != t6) for (let e8 = 0; e8 < this.parameters.maxTextureImageUnits; e8++) this._state.textureUnitMap[e8] === t6 && (this.bindTexture(null, e8), this._state.textureUnitMap[e8] = null);
  }
  ...
  bindFramebufferSeparate(t6, e8, s4 = false) {
    const i2 = e8 === n2.READ_FRAMEBUFFER, a2 = i2 ? this._state.readFramebuffer : this._state.drawFramebuffer;
    (s4 || a2 !== t6) && (null == t6 ? this.gl.bindFramebuffer(e8, null) : t6.initializeAndBind(e8), i2 ? this._state.readFramebuffer = t6 ?? null : (this._stateTracker.invalidateDrawBuffers(), this._state.drawFramebuffer = t6 ?? null));
  }
  blitFramebuffer(t6, e8, s4 = 0, i2 = 0, a2 = t6.width, r6 = t6.height, n7 = 0, l = 0, h2 = e8.width, f3 = e8.height, o5 = _.COLOR_BUFFER_BIT, d2 = L.NEAREST) {
    this.bindFramebufferSeparate(t6, n2.READ_FRAMEBUFFER), this.bindFramebufferSeparate(e8, n2.DRAW_FRAMEBUFFER), this.gl.blitFramebuffer(s4, i2, a2, r6, n7, l, h2, f3, o5, d2);
  }
  bindBuffer(t6, e8) {
    if (t6) switch (e8 ?? (e8 = t6.bufferType), e8) {
      case A2.ARRAY_BUFFER:
        this._state.vertexBuffer = N2(this.gl, t6, e8, this._state.vertexBuffer);
        break;
      case A2.ELEMENT_ARRAY_BUFFER:
        this._state.indexBuffer = N2(this.gl, t6, e8, this._state.indexBuffer);
        break;
      case A2.UNIFORM_BUFFER:
        this._state.uniformBuffer = N2(this.gl, t6, e8, this._state.uniformBuffer);
        break;
      case A2.PIXEL_PACK_BUFFER:
        this._state.pixelPackBuffer = N2(this.gl, t6, e8, this._state.pixelPackBuffer);
        break;
      case A2.PIXEL_UNPACK_BUFFER:
        this._state.pixelUnpackBuffer = N2(this.gl, t6, e8, this._state.pixelUnpackBuffer);
        break;
      case A2.COPY_READ_BUFFER:
        this._state.copyReadBuffer = N2(this.gl, t6, e8, this._state.copyReadBuffer);
        break;
      case A2.COPY_WRITE_BUFFER:
        this._state.copyWriteBuffer = N2(this.gl, t6, e8, this._state.copyWriteBuffer);
        break;
      case A2.TRANSFORM_FEEDBACK_BUFFER:
        this._state.transformFeedbackBuffer = N2(this.gl, t6, e8, this._state.transformFeedbackBuffer);
    }
  }
  bindRenderbuffer(t6) {
    const e8 = this.gl;
  ...
  _getBufferBinding(t6, e8) {
    if (e8 >= this.parameters.maxUniformBufferBindings || e8 < 0) return console.error("Uniform buffer binding point is out of range!"), null;
    const s4 = t6 === A2.UNIFORM_BUFFER ? this._state.uniformBufferBindingPoints : this._state.transformBufferBindingPoints;
    let i2 = s4[e8];
    return null == i2 && (i2 = { buffer: null, offset: 0, size: 0 }, s4[e8] = i2), i2;
  }
  bindBufferBase(t6, e8, s4) {
    const i2 = this._getBufferBinding(t6, e8);
    null != i2 && (i2.buffer === s4 && 0 === i2.offset && 0 === i2.size || (this.gl.bindBufferBase(t6, e8, s4 ? s4.glName : null), i2.buffer = s4, i2.offset = 0, i2.size = 0));
  }
  bindBufferRange(t6, e8, s4, i2, a2) {
    const r6 = this._getBufferBinding(t6, e8);
    null != r6 && (r6.buffer === s4 && r6.offset === i2 && r6.size === a2 || (i2 % this._parameters.uniformBufferOffsetAlignment == 0 ? (this.gl.bindBufferRange(t6, e8, s4.glName, i2, a2), r6.buffer = s4, r6.offset = i2, r6.size = a2) : console.error("Uniform buffer binding offset is not a multiple of the context offset alignment")));
  }
  bindUBO(t6, e8, s4, a2) {
    null != e8 ? (a() && (a2 ?? e8.byteLength) > this._parameters.maxUniformBlockSize && console.error("Attempting to bind more data than the maximum uniform block size"), e8.initialize(), void 0 !== s4 && void 0 !== a2 ? this.bindBufferRange(A2.UNIFORM_BUFFER, t6, e8.buffer, s4, a2) : this.bindBufferBase(A2.UNIFORM_BUFFER, t6, e8.buffer)) : this.bindBufferBase(A2.UNIFORM_BUFFER, t6, null);
  }
  unbindUBO(t6) {
  ...
  bindTransformFeedback(t6) {
    const { gl: e8 } = this;
    e8.bindTransformFeedback(e8.TRANSFORM_FEEDBACK, t6.glName);
  }
  beginTransformFeedback(t6, e8) {
    if (this._transformFeedbackRequestInfo) throw new Error("Already in a transform feedback request");
    const { gl: s4 } = this;
    s4.bindTransformFeedback(s4.TRANSFORM_FEEDBACK, t6.glName), s4.beginTransformFeedback(e8), this._transformFeedbackRequestInfo = { primitiveType: e8 };
  }
  ...
  enforceState() {
    ...
    for (let s4 = 0; s4 < this.parameters.maxVertexAttributes; s4++) t6.disableVertexAttribArray(s4);
...
//# sourceMappingURL=chunk-HOELAGXK.js.map
