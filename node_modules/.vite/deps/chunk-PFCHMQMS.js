...
function p(n) {
  ...
  const d3 = { avg: r2, count: m2, max: a3, min: i2, stddev: u2, sum: o2, variance: s2 };
  return l2 && (d3.nullcount = e.length - m2), n.percentileParams && (d3.median = v(e, n.percentileParams)), d3;
}
function v(n, e) {
  const { fieldType: t2, value: l2, orderBy: i2, isDiscrete: a3 } = e, o2 = T(t2, "desc" === i2);
...
function q(e) {
  const { breakCount: t2, field: a3, normalizationField: o2, normalizationType: r2 } = e, u2 = e.classificationMethod || l, s2 = "standard-deviation" === u2 ? e.standardDeviationInterval || i : void 0;
  return new d({ breakCount: t2, classificationField: a3, classificationMethod: u2, normalizationField: "field" === r2 ? o2 : void 0, normalizationType: r2, standardDeviationInterval: s2 });
}
function P(n, e) {
  var _a, _b;
  let t2 = n.classBreaks;
  const l2 = t2.length, i2 = (_a = t2[0]) == null ? void 0 : _a.minValue, a3 = (_b = t2[l2 - 1]) == null ? void 0 : _b.maxValue, o2 = "standard-deviation" === e, u2 = r;
  return t2 = t2.map((n2) => {
...
function U(n, e) {
  const t2 = G(n, e);
  if (null == t2.min && null == t2.max) return { bins: [], minValue: t2.min, maxValue: t2.max, normalizationTotal: e.normalizationTotal };
  const l2 = t2.intervals, i2 = t2.min ?? 0, a3 = t2.max ?? 0, o2 = l2.map((n2, e2) => ({ minValue: l2[e2][0], maxValue: l2[e2][1], count: 0 }));
  for (const r2 of n) if (null != r2 && r2 >= i2 && r2 <= a3) {
    const n2 = L(l2, r2);
    n2 > -1 && o2[n2].count++;
  }
  return { bins: o2, minValue: i2, maxValue: a3, normalizationTotal: e.normalizationTotal };
}
function G(n, e) {
  const { field: t2, classificationMethod: l2, standardDeviationInterval: i2, normalizationType: a3, normalizationField: r2, normalizationTotal: u2, minValue: s2, maxValue: m2 } = e, c2 = e.numBins || o;
  let d3 = null, v2 = null, T2 = null;
  if ((!l2 || "equal-interval" === l2) && !a3) {
    if (null != s2 && null != m2) d3 = s2, v2 = m2;
    else {
      const e2 = p({ values: n, minValue: s2, maxValue: m2, useSampleStdDev: !a3, supportsNullCount: f({ normalizationType: a3, normalizationField: r2, minValue: s2, maxValue: m2 }) });
      d3 = e2.min ?? null, v2 = e2.max ?? null;
    }
    T2 = j(d3 ?? 0, v2 ?? 0, c2);
  } else {
    const { classBreaks: e2 } = E(n, { field: t2, normalizationType: a3, normalizationField: r2, normalizationTotal: u2, classificationMethod: l2, standardDeviationInterval: i2, minValue: s2, maxValue: m2, numClasses: c2 });
...
//# sourceMappingURL=chunk-PFCHMQMS.js.map
