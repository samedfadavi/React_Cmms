...
async function m2(t2, e3, r2) {
  const { geometry: o3 } = e3, i4 = { ...e3.attributes };
  if (null != r2 && "mesh" === (o3 == null ? void 0 : o3.type)) {
    ...
    if (!(g3 && y4 && h2 || !g3 && !y4 && R2)) return null;
    const I2 = Q(c3, p, b3);
...
async function f2(t2, e3) {
  const r2 = await Promise.all((e3.addAttachments ?? []).map((e4) => g2(t2, e4))), a4 = await Promise.all((e3.updateAttachments ?? []).map((e4) => g2(t2, e4))), o3 = e3.deleteAttachments ?? [];
  return r2.length || a4.length || o3.length ? { adds: r2, updates: a4, deletes: [...o3] } : null;
}
async function g2(t2, e3) {
  var _a;
  const { feature: r2, attachment: a4 } = e3, { globalId: s4, name: n3, contentType: l, data: i4, uploadId: u } = a4, d3 = { globalId: s4 };
  if (r2 && ("attributes" in r2 ? d3.parentGlobalId = (_a = r2.attributes) == null ? void 0 : _a[t2.globalIdField] : r2.globalId && (d3.parentGlobalId = r2.globalId)), u) d3.uploadId = u;
  else if (i4) {
    const t3 = await Wt(i4);
...
function b2(t2, e3, r2) {
  ...
  if (r2 && $(e3)) return e3.map((t3) => t3.globalId);
  if (v(e3)) return e3.map((t3) => t3.objectId);
  const a4 = r2 ? t2.globalIdField : t2.objectIdField;
  return a4 ? e3.map((t3) => t3.getAttribute(a4)) : [];
}
function y3(t2) {
  var _a, _b, _c;
  const e3 = t2 == null ? void 0 : t2.assetMaps;
  if (e3) {
    for (const t3 of e3.addResults) t3.success || n.getLogger("esri.layers.graphics.sources.support.sourceUtils").error(`Failed to map asset to feature with globalId ${t3.globalId}.`);
    for (const t3 of e3.updateResults) t3.success || n.getLogger("esri.layers.graphics.sources.support.sourceUtils").error(`Failed to map asset to feature with globalId ${t3.globalId}.`);
  }
  const a4 = t2 == null ? void 0 : t2.attachments, o3 = { addFeatureResults: ((_a = t2 == null ? void 0 : t2.addResults) == null ? void 0 : _a.map(R)) ?? [], updateFeatureResults: ((_b = t2 == null ? void 0 : t2.updateResults) == null ? void 0 : _b.map(R)) ?? [], deleteFeatureResults: ((_c = t2 == null ? void 0 : t2.deleteResults) == null ? void 0 : _c.map(R)) ?? [], addAttachmentResults: (a4 == null ? void 0 : a4.addResults) ? a4.addResults.map(R) : [], updateAttachmentResults: (a4 == null ? void 0 : a4.updateResults) ? a4.updateResults.map(R) : [], deleteAttachmentResults: (a4 == null ? void 0 : a4.deleteResults) ? a4.deleteResults.map(R) : [] };
  return (t2 == null ? void 0 : t2.editMoment) && (o3.editMoment = t2.editMoment), o3;
}
function R(t2) {
...
function h(e3, r2) {
  return new d2({ attributes: e3.attributes, geometry: y2({ ...e3.geometry, spatialReference: r2 }) });
}
function I(t2, e3) {
  var _a, _b, _c;
  return { adds: ((_a = t2 == null ? void 0 : t2.adds) == null ? void 0 : _a.map((t3) => h(t3, e3))) || [], updates: ((_b = t2 == null ? void 0 : t2.updates) == null ? void 0 : _b.map((t3) => ({ original: h(t3[0], e3), current: h(t3[1], e3) }))) || [], deletes: ((_c = t2 == null ? void 0 : t2.deletes) == null ? void 0 : _c.map((t3) => h(t3, e3))) || [], spatialReference: e3 };
}
function j(t2) {
...
var G2 = class extends m {
  constructor(e3) {
    super(e3), this.type = "feature-layer", this.supportedSourceTypes = /* @__PURE__ */ new Set(["Feature Layer", "Oriented Imagery Layer", "Table", "Catalog Layer"]), this.refresh = k(async () => {
      var _a, _b;
      await this.load();
      ...
      const t2 = e4 !== ((_b = this.sourceJSON.editingInfo) == null ? void 0 : _b.lastEditDate);
  ...
  load(e3) {
    const t2 = this.layer.sourceJSON, r2 = this._fetchService(t2, { ...e3 }).then(() => this.layer.setUserPrivileges(this.sourceJSON.serviceItemId, e3)).then(() => this._ensureLatestMetadata(e3));
  ...
  async applyEdits(e3, t2) {
    ...
    const { layer: s4 } = this;
    await T(s4, "editing");
    const i4 = "infoFor3D" in s4 ? s4.infoFor3D : null, o3 = null != i4, u = o3 || ((t2 == null ? void 0 : t2.globalIdUsed) ?? false), l = o3 ? await this._uploadMeshesAndGetAssetMapEditsJSON(e3) : null, c3 = ((_a = e3.addFeatures) == null ? void 0 : _a.map((e4) => m2(this.layer, e4, i4))) ?? [], d3 = (await Promise.all(c3)).filter(O), p = ((_b = e3.updateFeatures) == null ? void 0 : _b.map((e4) => m2(this.layer, e4, i4))) ?? [], h2 = (await Promise.all(p)).filter(O), y4 = b2(this.layer, e3.deleteFeatures, u);
    i2(d3, h2, s4.spatialReference);
    const m3 = await f2(this.layer, e3), f3 = s4.capabilities.editing.supportsAsyncApplyEdits && o3, g3 = (t2 == null ? void 0 : t2.gdbVersion) || s4.gdbVersion, w2 = { gdbVersion: g3, rollbackOnFailure: t2 == null ? void 0 : t2.rollbackOnFailureEnabled, useGlobalIds: u, returnEditMoment: t2 == null ? void 0 : t2.returnEditMoment, usePreviousEditMoment: t2 == null ? void 0 : t2.usePreviousEditMoment, async: f3 };
    await i3(this.layer.url, g3, true);
    const S = c2(this.layer.url, g3 || null);
    if (await o2(s4.url, g3, s4.historicMoment)) throw new s2("feature-layer-source:historic-version", "Editing a historic version is not allowed");
    (t2 == null ? void 0 : t2.returnServiceEditsOption) ? (w2.edits = JSON.stringify([{ id: s4.layerId, adds: d3.length ? d3 : null, updates: h2.length ? h2 : null, deletes: y4.length ? y4 : null, attachments: m3, assetMaps: l }]), w2.returnServiceEditsOption = V2.toJSON(t2 == null ? void 0 : t2.returnServiceEditsOption), w2.returnServiceEditsInSourceSR = t2 == null ? void 0 : t2.returnServiceEditsInSourceSR) : (w2.adds = d3.length ? JSON.stringify(d3) : null, w2.updates = h2.length ? JSON.stringify(h2) : null, w2.deletes = y4.length ? u ? JSON.stringify(y4) : y4.join(",") : null, w2.attachments = m3 && JSON.stringify(m3), w2.assetMaps = null != l ? JSON.stringify(l) : void 0);
    ...
    const E = (t2 == null ? void 0 : t2.returnServiceEditsOption) ? s4.url : s4.parsedUrl.path;
    let O2;
    try {
      O2 = f3 ? await this._asyncApplyEdits(E + "/applyEdits", q) : await U(E + "/applyEdits", q);
    } catch (_) {
      if (!j(_)) throw _;
      q.authMode = "immediate", O2 = f3 ? await this._asyncApplyEdits(E + "/applyEdits", q) : await U(E + "/applyEdits", q);
    }
  ...
  async deleteAttachments(e3, t2) {
    ...
    try {
      return (await U(i4, this._getLayerRequestOptions({ query: { attachmentIds: t2.join(",") }, method: "post" }))).data.deleteAttachmentResults.map(R);
    } catch (n3) {
  ...
  fetchRecomputedExtents(e3 = {}) {
    ...
    return this.load({ signal: t2 }).then(async () => {
      const t3 = this._getLayerRequestOptions({ ...e3, query: { returnUpdates: true } }), { layerId: a4, url: i4 } = this.layer, { data: n3 } = await U(`${i4}/${a4}`, t3), { id: o3, extent: u, fullExtent: l, timeExtent: c3 } = n3, d3 = u || l;
  ...
  async queryFeatures(e3, t2) {
    var _a;
    await this.load();
    const r2 = await this.queryTask.execute(e3, { ...t2, query: this._createRequestQueryOptions(t2) });
    return ((_a = e3.outStatistics) == null ? void 0 : _a.length) && r2.features.length && r2.features.forEach((t3) => {
      var _a2;
  ...
  async uploadAssets(e3, t2) {
    const { uploadAssets: r2 } = await import("./uploadAssets-K7KLBHXP.js");
  ...
  async _asyncApplyEdits(e3, t2) {
    const s4 = (await U(e3, t2)).data.statusUrl;
    for (; ; ) {
      const e4 = (await U(s4, { query: { f: "json" }, responseType: "json" })).data;
      switch (e4.status) {
        case "Completed":
  ...
  async _fetchService(e3, t2) {
    if (!e3) {
      const s5 = {};
      has("featurelayer-advanced-symbols") && (s5.returnAdvancedSymbols = true), (t2 == null ? void 0 : t2.cacheBust) && (s5._ts = Date.now());
      const { data: a4 } = await U(this.layer.parsedUrl.path, this._getLayerRequestOptions({ query: s5, signal: t2 == null ? void 0 : t2.signal }));
      e3 = a4;
    }
  ...
  async _patchServiceJSON(e3, t2) {
    ...
    if ("esriGeometryMultiPatch" === e3.geometryType && e3.infoFor3D && (e3.geometryType = "mesh"), null == e3.extent) try {
      const { data: s4 } = await U(this.layer.url, this._getLayerRequestOptions({ signal: t2 }));
  ...
  async _uploadMeshesAndGetAssetMapEditsJSON(e3) {
    ...
    for (const i4 of t2) {
      ...
      if (a3(t3)) s4.push(e4);
        ...
        const t4 = e4.anchor, { convertMeshVertexSpace: r3 } = await import("./convertMeshVertexSpace-RX3EVH42.js"), n3 = await r3(e4, new i({ origin: [t4.x, t4.y, t4.z ?? 0] }));
  ...
  _getAssetMapEditsJSON(e3) {
    ...
    for (const a4 of e3) {
      ...
      for (const r3 of u) 1 === r3.parts.length ? t2.push({ globalId: r(), parentGlobalId: o3, assetName: r3.assetName, assetHash: r3.parts[0].partHash, flags: [] }) : n.getLogger(this).error(`Skipping asset ${r3.assetName}. It does not have exactly one part, so we cannot map it to a feature.`);
    }
    return t2;
  }
  _createEditsResult(e3) {
    ...
    let a4 = null;
    if (Array.isArray(t2)) for (const n3 of t2) s4.push({ id: n3.id, editedFeatures: n3.editedFeatures }), n3.id === r2 && (a4 = { addResults: n3.addResults ?? [], updateResults: n3.updateResults ?? [], deleteResults: n3.deleteResults ?? [], attachments: n3.attachments, editMoment: n3.editMoment });
    else a4 = t2;
    const i4 = y3(a4);
    if (s4.length > 0) {
      i4.editedFeatureResults = [];
      for (const e4 of s4) {
        const { editedFeatures: t3 } = e4, r3 = (t3 == null ? void 0 : t3.spatialReference) ? new f(t3.spatialReference) : null;
  ...
  _createAttachmentErrorResult(e3, t2) {
    var _a;
    const r2 = ((_a = t2.details.messages) == null ? void 0 : _a[0]) || t2.message, s4 = t2.details.httpStatus || t2.details.messageCode;
  ...
  _getLayerRequestOptions(e3 = {}) {
    ...
    return { ...e3, query: { gdbVersion: s4, layer: "dynamicDataSource" in t2 && t2.dynamicDataSource ? JSON.stringify({ source: t2.dynamicDataSource }) : void 0, ...r2.query, f: "json", ...this._createRequestQueryOptions(e3) }, responseType: "json" };
  }
  async _areAllAssetsAlreadyMapped(e3) {
    ...
    const { assetMapFieldRoles: { parentGlobalId: c3, assetHash: d3 } } = i4, p = new b({ where: `${c3} IN (${l.map((e4) => `'${e4}'`)})`, outFields: [d3, c3], returnGeometry: false }), h2 = await s3(o3, p), { features: y4 } = h2;
...
//# sourceMappingURL=chunk-2RV34MGI.js.map
