...
function n3(n5) {
  const i4 = t4(s3(n5)), o5 = i4, a3 = true, l6 = Math.max(i4 / 2, 5), r2 = Math.round(u(n5.maxPathLength) / l6) + 1, u5 = 10, { density: c6 } = n5;
  return { smoothing: u(n5.smoothing), interpolate: true, velocityScale: "flow-from" === n5.flowRepresentation ? 1 : -1, verticesPerLine: r2, minSpeedThreshold: 1e-3, segmentLength: l6, maxTurnAngle: 1, collisions: a3, lineCollisionWidth: o5, lineSpacing: u5, density: c6 };
}
function t4(e5) {
...
function a2(e5) {
  if (!e5.hasVisualVariables("color")) return { kind: "constant", value: i2(e5.color) };
...
function l2(e5) {
  if (!e5.hasVisualVariables("opacity")) return { kind: "constant", value: [1] };
...
var f3 = class _f {
  ...
  async load(a3, r2) {
    const { extent: e5, size: s6 } = a3;
    s(r2);
    const o5 = await this._params.loadImagery(e5, s6[0], s6[1], this._params.timeExtent, r2);
    return new x(o5, { color: this._params.color, opacity: this._params.opacity });
  }
  render(t7, a3, e5) {
    const { context: s6 } = t7, { program: n5 } = e5;
...
var w2 = { vsPath: "raster/flow/imagery", fsPath: "raster/flow/imagery", attributes: d3 };
var x = class {
  constructor(t7, a3) {
    this._flowData = t7, this._values = a3;
  }
  attach(t7) {
    const { context: a3 } = t7, { width: r2, height: s6 } = this._flowData, o5 = c.createVertex(a3, F.STATIC_DRAW, new Uint16Array([0, 0, 0, 1, r2, 0, 1, 1, 0, s6, 0, 0, r2, s6, 1, 0])), i4 = new o2(a3, d3, u3, { geometry: o5 }), _5 = [];
...
var _3 = class __ {
  ...
  async load(e5, a3) {
    const { extent: s6, size: r2 } = e5;
    s(a3);
    const i4 = await this._params.loadImagery(s6, r2[0], r2[1], this._params.timeExtent, a3), { vertexData: o5, indexData: n5 } = await this._params.createFlowMesh("Particles", this._params.simulationSettings, i4, a3);
    return new c4(o5, n5, { color: this._params.color, opacity: this._params.opacity, size: this._params.size });
  }
  render(t7, e5, s6) {
    const { context: n5 } = t7, { program: m3 } = s6;
...
var c4 = class {
  ...
  attach(t7) {
    const { context: e5 } = t7, a3 = c.createVertex(e5, F.STATIC_DRAW, this._vertexData), r2 = c.createIndex(e5, F.STATIC_DRAW, this._indexData), i4 = new o2(e5, l3, h3, { geometry: a3 }, r2), o5 = [];
...
var t5 = class _t {
  ...
  async load(t7, e5) {
    const r2 = await Promise.all(this._styles.map((s6) => s6.load(t7, e5)));
    return new s4(r2);
  }
  render(t7, s6, e5) {
...
var _4 = class __ {
  ...
  async load(t7, a3) {
    const { extent: s6, size: r2 } = t7;
    s(a3);
    const i4 = await this._params.loadImagery(s6, r2[0], r2[1], this._params.timeExtent, a3), { vertexData: o5, indexData: n5 } = await this._params.createFlowMesh("Streamlines", this._params.simulationSettings, i4, a3);
    return new d4(o5, n5, { color: this._params.color, opacity: this._params.opacity, size: this._params.size });
  }
  render(e5, t7, s6) {
    const { context: n5 } = e5, { program: m3 } = s6;
...
var d4 = class {
  ...
  attach(e5) {
    const { context: t7 } = e5, a3 = c.createVertex(t7, F.STATIC_DRAW, this._vertexData), r2 = c.createIndex(t7, F.STATIC_DRAW, this._indexData), i4 = new o2(t7, l4, h4, { geometry: a3 }, r2), o5 = [];
    "ramp" === this._values.color.kind && o5.push("vvColor"), "ramp" === this._values.opacity.kind && o5.push("vvOpacity"), "ramp" === this._values.size.kind && o5.push("vvSize");
...
var y2 = 2.3;
function g2(g3, w3) {
  const { flowSpeed: h7, trailLength: v } = g3, S2 = n3(g3);
  let j = null;
  ...
  if ("none" === g3.background) k.color = x2;
    ...
    const e5 = { loadImagery: w3.loadImagery, timeExtent: w3.timeExtent, color: x2, opacity: { kind: "constant", value: [1] } };
    j = new f3(e5), k.color = o3();
  }
  const I2 = { loadImagery: w3.loadImagery, createFlowMesh: w3.createFlowMesh, simulationSettings: S2, timeExtent: w3.timeExtent, trailLength: v, flowSpeed: h7, featheringSize: u4, featheringOffset: f6, introFade: p2, fadeToZero: d5, decayRate: y2, color: k.color, opacity: k.opacity, size: k.size }, z = "butt" === g3.trailCap || t4(s3(g3)) <= m ? new _4(I2) : new _3(I2);
...
var s5 = class extends t3 {
  constructor() {
    super(...arguments), this._visualState = { time: 0, dvsMat3: e3(), displayViewMat3: e3() };
  }
  dispose() {
  }
  prepareState(t7) {
  ...
  draw(t7, e5) {
    const { requestRender: a3, allowDelayedRender: s6 } = t7, { displayData: i4 } = e5;
    if (null == i4) return;
...
var t6 = class extends n2 {
  ...
  prepareRenderPasses(s6) {
    const t7 = s6.registerRenderPass({ name: "flow", brushes: [s5], target: () => this.children, drawPhase: E2.MAP });
...
var e4 = class {
  constructor(t7, e5, s6, a3) {
    this.state = { name: "created" }, this.flowStyle = t7, this.extent = e5, this.size = s6, this.pixelRatio = a3;
  }
  async load() {
    const t7 = new AbortController();
    this.state = { name: "loading", abortController: t7 };
    const e5 = { extent: this.extent, size: this.size, pixelRatio: this.pixelRatio }, s6 = await this.flowStyle.load(e5, t7.signal);
  ...
  attach(e5) {
    ...
    s6.attach(e5), this.state = { name: "attached", resources: s6 };
  }
  detach() {
...
var o4 = class extends s2 {
  ...
  setTransform(r2) {
    ...
    const m3 = (l6.extent.xmax - l6.extent.xmin) / l6.size[0] / r2.resolution, c6 = h(r2.rotation), { displayViewScreenMat3: x2 } = this.transforms;
...
var d6 = class extends S {
  ...
  update(t7) {
    ...
    const { extent: e5, rotation: a3, resolution: r2, pixelRatio: l6 } = t7.state, p3 = f7(e5, a3);
    p3.expand(m2);
...
var h5 = d6;
function f7(t7, o5) {
  const i4 = new _({ x: (t7.xmax + t7.xmin) / 2, y: (t7.ymax + t7.ymin) / 2, spatialReference: t7.spatialReference }), s6 = t7.xmax - t7.xmin, a3 = t7.ymax - t7.ymin, r2 = Math.abs(Math.cos(h(o5))), n5 = Math.abs(Math.sin(h(o5))), p3 = r2 * s6 + n5 * a3, m3 = n5 * s6 + r2 * a3, d9 = new w({ xmin: i4.x - p3 / 2, ymin: i4.y - m3 / 2, xmax: i4.x + p3 / 2, ymax: i4.y + m3 / 2, spatialReference: t7.spatialReference });
...
var h6 = class extends S {
  constructor() {
    super(...arguments), this._loadImagery = (t7, e5, i4, o5, r2) => g(this.layer, t7, e5, i4, o5, r2), this._createFlowMesh = (t7, e5, i4, o5) => this.layer.createFlowMesh({ meshType: t7, flowData: i4, simulationSettings: e5 }, { signal: o5 }), this.attached = false, this.type = "flow", this.timeExtent = null, this.redrawOrRefetch = async () => {
...
var d8 = class extends t3 {
  ...
  draw(e5, a3) {
    ...
    const { context: t7, timeline: s6 } = e5;
  ...
  _drawTriangles(e5, a3, t7, r2) {
    const { context: n5, painter: i4, requestRender: d9, allowDelayedRender: m3 } = e5, { symbolizerParameters: u5 } = a3, f9 = u5.dataRange ? ["dataRange"] : [];
    ...
    n5.useProgram(_5);
    const { coordScale: g3, opacity: p3, transforms: y3 } = a3;
  ...
  _drawScalars(e5, a3, t7, r2) {
    const { context: n5, painter: i4, requestRender: l6, allowDelayedRender: d9 } = e5, { symbolizerParameters: m3 } = a3, u5 = [];
  ...
  _getSymbolSize(e5, a3) {
    const t7 = a3.key ? 2 ** (e5.displayLevel - a3.key.level) : a3.resolution / e5.state.resolution, { symbolTileSize: r2 } = a3.symbolizerParameters;
...
var f8 = class extends s2 {
  ...
  _createVectorFieldVAO(t7, e5) {
    const { vertexData: a3, indexData: s6 } = e5, i4 = c.createVertex(t7, F.STATIC_DRAW, new Float32Array(a3)), r2 = c.createIndex(t7, F.STATIC_DRAW, new Uint32Array(s6)), o5 = b3("vector-field", { geometry: [{ location: 0, name: "a_pos", count: 2, type: C.FLOAT, normalized: false }, { location: 1, name: "a_offset", count: 2, type: C.FLOAT, normalized: false }, { location: 2, name: "a_vv", count: 2, type: C.FLOAT, normalized: false }] });
    return { vao: new o2(t7, o5.attributes, o5.bufferLayouts, { geometry: i4 }, r2), elementCount: s6.length };
  }
};
...
//# sourceMappingURL=chunk-QUSFRO4I.js.map
