...
g = function() {
  ...
  function n(t3, r3) {
    ...
    function Dt(e4) {
      ...
      for (n3 = { line: (n3 = It[r4]).line, column: n3.column }; r4 < e4; ) 10 === t3.charCodeAt(r4) ? (n3.line++, n3.column = 1) : n3.column++, r4++;
      return It[e4] = n3, n3;
    }
    function Gt(t4, e4, r4) {
      var n3 = Dt(t4), u2 = Dt(e4);
      return { source: a2, start: { offset: t4, line: n3.line, column: n3.column }, end: { offset: e4, line: u2.line, column: u2.column } };
    }
    function Jt(t4) {
...
function y(t2) {
  ...
  const e3 = t2.map((t3) => {
    if (!Number.isFinite(t3.scale) || t3.scale <= 0) throw new s("effect:invalid-scale", "scale must be finite and greater than 0", { stop: t3 });
    return { scale: t3.scale, effects: A(t3.value) };
  });
  e3.sort((t3, e4) => e4.effects.length - t3.effects.length);
  for (let r2 = 0; r2 < e3.length - 1; r2++) {
    if (!t(e3[r2].effects, e3[r2 + 1].effects)) throw new s("effect:interpolation-impossible", "Cannot interpolate by scale between 2 lists of mixed effects", { a: e3[r2].effects, b: e3[r2 + 1].effects });
...
function E(t2) {
  ...
  for (const n of t2.parameters) if ("color" === n.type) {
    ...
    const t3 = U(n);
    if (Object.isFrozen(e3)) throw new s("effect:type-error", "<length> parameters not consecutive", { lengths: e3 });
    e3.push(t3), 3 === e3.length && k(t3, n);
  }
  if (e3.length < 2 || e3.length > 3) throw new s("effect:type-error", `Expected <length>{2,3}, Actual: <length>{${e3.length}}`, { lengths: e3 });
...
function $(t2, e3) {
  if (t2.length > e3) throw new s("effect:type-error", `Function supports up to ${e3} parameters, Actual: ${t2.length}`, { parameters: t2 });
}
function O(t2) {
...
function k(t2, e3) {
  if (t2 < 0) throw new s("effect:type-error", `Negative values are not allowed, Actual: ${t2}`, { term: e3 });
}
function q(t2) {
  if ("quantity" !== t2.type || null !== t2.unit) throw new s("effect:type-error", `Expected <double>, Actual: ${O(t2)}`, { term: t2 });
}
function S(t2) {
  if ("quantity" !== t2.type || null !== t2.unit && "%" !== t2.unit) throw new s("effect:type-error", `Expected <double> or <percentage>, Actual: ${O(t2)}`, { term: t2 });
}
var z = { deg: 1, grad: 0.9, rad: 180 / Math.PI, turn: 360 };
function R(t2) {
  if ("quantity" !== t2.type || !(0 === t2.value && null === t2.unit || t2.unit && null != z[t2.unit])) throw new s("effect:type-error", `Expected <angle>, Actual: ${O(t2)}`, { term: t2 });
}
var I = { px: 1, cm: 96 / 2.54, mm: 96 / 2.54 / 10, in: 96, pc: 16, pt: 96 / 72 };
function M(t2) {
  if ("quantity" !== t2.type || !(0 === t2.value && null === t2.unit || t2.unit && null != I[t2.unit])) throw new s("effect:type-error", `Expected <length>, Actual: ${O(t2)}`, { term: t2 });
}
function P(t2) {
  S(t2);
  const e3 = t2.value;
...
function _(t2) {
  if (!r(t2)) throw new s("effect:unknown-color", `color '${t2}' isn't valid`, { namedColor: t2 });
  return a(t2);
}
var B = /^rgba?/i;
var D = /^hsla?/i;
...
//# sourceMappingURL=chunk-QUNQAVSE.js.map
