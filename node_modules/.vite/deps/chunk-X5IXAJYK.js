...
var R = (e, t, n, i = 0) => {
  ...
  const { cols: l, rows: c } = t, f2 = (r - o) / l / n[0], u = (a2 - s4) / c / n[1], m = 2 * i;
...
var S2 = { 3395: 20037508342789244e-9, 3410: 17334193943686873e-9, 3857: 20037508342788905e-9, 3975: 17367530445161372e-9, 4087: 20037508342789244e-9, 4088: 20015108787169147e-9, 6933: 17367530445161372e-9, 32662: 20037508342789244e-9, 53001: 2001508679602057e-8, 53002: 1000754339801029e-8, 53003: 2001508679602057e-8, 53004: 2001508679602057e-8, 53016: 14152803599503474e-9, 53017: 17333573624304302e-9, 53034: 2001508679602057e-8, 53079: 20015114352186374e-9, 53080: 20015114352186374e-9, 54001: 20037508342789244e-9, 54002: 10018754171394624e-9, 54003: 20037508342789244e-9, 54004: 20037508342789244e-9, 54016: 14168658027268292e-9, 54017: 1736753044516137e-8, 54034: 20037508342789244e-9, 54079: 20037508342789244e-9, 54080: 20037508342789244e-9, 54100: 20037508342789244e-9, 54101: 20037508342789244e-9 };
var P2 = 32;
var b = 4;
var G = b;
...
function C(e, t, i, o = null) {
  ...
  w2(r, t, o);
  const s4 = i.center, a2 = new w({ xmin: s4.x - e.x / 2, xmax: s4.x + e.x / 2, ymin: s4.y - e.y / 2, ymax: s4.y + e.y / 2, spatialReference: r }), l = K(a2, t, o), c = D(t);
  let f2;
  if (null == l || null != c && l.width >= c) {
    const i2 = W(r) / W(t);
    f2 = { x: e.x * i2, y: e.y * i2 };
  } else f2 = { x: l.width, y: l.height };
  return f2;
}
...
function I(e) {
  const { points: t } = F(e, null), { inSR: n, outSR: i, datumTransformation: o } = e, r = t.map((e2) => new _(e2[0], e2[1], n)), s4 = K(r, i, o);
  return o && z(r, s4, n, i), s4.map((e2) => e2 ? [e2.x, e2.y] : [NaN, NaN]);
}
function W3(e) {
  const { inSR: t, outSR: n, datumTransformation: a2 } = e, l = A(t), { points: c, mask: f2 } = F(e, l);
  if (!t.isGeographic) {
...
function O2(e) {
  const { cols: t, rows: n, xres: i, yres: o, usePixelCenter: r, inSR: s4, outSR: a2 } = e;
  let { xmin: l, ymax: c } = e;
  r && (l += i / 2, c -= o / 2);
  const f2 = [], u = [], x = Math.max(t, n);
  for (let g = 0; g < x; g++) {
    const e2 = l + i * Math.min(t, g), r2 = c - o * Math.min(n, g), x2 = K(new _({ x: e2, y: r2, spatialReference: s4 }), a2);
...
function A(e, t = false) {
  ...
  return k.set(n, { pcs: s4, gcs: a2 }), t ? a2 : s4;
}
function B(e, t = 0, n = false) {
  ...
  return { isEnvelope: false, isPannable: c, vertices: f2, coef: m, bbox: [x, h, p, g], poleLocation: l };
}
function F(e, t) {
  const n = [], { cols: i, rows: o, xres: r, yres: s4, usePixelCenter: a2 } = e;
  let { xmin: l, ymax: c } = e;
...
function q(e, t) {
  ...
  let { xmin: i, xmax: o, ymin: r, ymax: s4 } = e[0];
  for (let a2 = 1; a2 < e.length; a2++) {
    const t2 = e[a2];
    o = t2.xmax + n * a2, r = Math.min(r, t2.ymin), s4 = Math.max(s4, t2.ymax);
  }
  return new w({ xmin: i, xmax: o, ymin: r, ymax: s4, spatialReference: e[0].spatialReference });
}
function J2(t, n, i = null, o = true) {
  ...
  if (1 === f2.length && t.xmax < a2 && t.xmax - a2 / 2 > _3(r)) {
    ...
    for (let i2 = 0; i2 <= s4; i2++) {
      ...
      f2[i2] = new w({ xmin: o2, xmax: l2, ymin: t.ymin, ymax: t.ymax, spatialReference: r });
    }
  }
...
function X(e, t) {
  ...
  let { xmin: i, ymin: o, xmax: r, ymax: s4 } = e;
  const a2 = e.spatialReference, l = new j({ spatialReference: a2, rings: [[[i, o], [r, o], [r, s4], [i, s4], [i, o]]] }), c = K(l, t);
  if (2 !== c.rings.length || !c.rings[0].length || !c.rings[1].length) return null;
  const { rings: f2 } = c, u = _3(a2), x = new w({ spatialReference: t });
  for (let m = 0; m < 2; m++) {
...
function K2(e, t, n = null, i = true, o = true) {
  ...
  if (Math.abs(s4.xmin - l[0]) < u && Math.abs(s4.xmax - l[1]) < u) {
    ...
    if (i2 < c && o2 > f2) {
      ...
      const o3 = i3.map((e2) => j2(e2, t, n)).filter((e2) => !isNaN(e2 == null ? void 0 : e2.x)).map((e2) => e2.x);
...
function Z(e) {
  ...
  const i = n / 2, { nativePixelSize: o, storageInfo: r, extent: s4 } = e, { maximumPyramidLevel: a2, blockWidth: l, pyramidScalingFactor: c } = r;
  let f2 = o.x;
  ...
  for (let p = 0; p <= a2; p++) {
    ...
    u.push({ resolutionX: f2, blockWidth: l, datasetColumnCount: n2, worldColumnCountFromOrigin: o2, leftMargin: a3, rightPadding: m2, originColumnOffset: r2 }), f2 *= c;
  }
...
function ee(e) {
  ...
  let n = e.spacing || [P2, P2], i = te(e), o = { cols: i.size[0] + 1, rows: i.size[1] + 1 };
  ...
  if (t && (r || a2 > G)) {
    M(l, c, e.datumTransformation) && (l.isGeographic || A(l)), n = [b, b], i = te({ ...e, spacing: n }), o = { cols: i.size[0] + 1, rows: i.size[1] + 1 }, s4 = R(i.offsets, o, n, G);
  }
  if (i.error = s4, n[0] > 1 && (i.coefficients = ne(i.offsets, o, r)), e.includeGCSGrid && !l.isGeographic && !l.isWebMercator) if (c.isGeographic) i.gcsGrid = { offsets: i.offsets, coefficients: i.coefficients, spacing: n };
  else {
    const t2 = A(l);
    if (null != t2 && !t2.isEnvelope) {
      const t3 = $(l), s5 = J2(e.projectedExtent, t3), { offsets: a3 } = te({ ...e, srcBufferExtent: s5, spacing: n }), c2 = ne(a3, o, r);
...
function te(e) {
  const { projectedExtent: t, srcBufferExtent: n, pixelSize: i, datumTransformation: o, rasterTransform: r } = e, s4 = t.spatialReference, a2 = n.spatialReference, l = w2(s4, a2), { xmin: c, ymin: f2, xmax: u, ymax: m } = t, x = D(a2), h = null != x && (e.hasWrapAround || "gcs-shift" === (r == null ? void 0 : r.type)), g = e.spacing || [P2, P2], y = g[0] * i.x, d2 = g[1] * i.y, M2 = 1 === g[0], R2 = Math.ceil((u - c) / y - 0.1 / g[0]) + (M2 ? 0 : 1), S3 = Math.ceil((m - f2) / d2 - 0.1 / g[1]) + (M2 ? 0 : 1), G2 = L({ cols: R2, rows: S3, xmin: c, ymax: m, xres: y, yres: d2, inSR: s4, outSR: a2, datumTransformation: o, preferPE: g[0] <= b, usePixelCenter: M2 }), k2 = [];
  let N2, T2 = 0;
  const v2 = M2 ? -1 : NaN, { xmin: C2, xmax: j3, ymax: z2, width: I2, height: W4 } = n, O3 = _3(a2, E2), B2 = null != x && C2 > 0 && j3 > x / 2;
  let F2 = false;
  if (l) {
    const e2 = A(s4);
  ...
  return { offsets: k2, error: null, coefficients: null, outofBoundPointCount: T2, spacing: g, size: M2 ? [R2, S3] : [R2 - 1, S3 - 1] };
}
function ne(e, t, n) {
  const { cols: i, rows: o } = t, r = new Float32Array((i - 1) * (o - 1) * 2 * 6), s4 = new Float32Array([-0, -1, 1, -1, 1, -0, 1, -0, -0]), a2 = new Float32Array([-1, 1, 0, 0, -1, 1, 1, 0, 0]);
  for (let l = 0; l < i - 1; l++) {
...
function oe(e) {
  const { spatialReference: t } = e, n = s3(t);
  if (!n) return e;
  ...
  return new w({ spatialReference: e.spatialReference, xmin: e.xmin + s4 * r, ymin: e.ymin, xmax: e.xmax + s4 * r, ymax: e.ymax });
}
function re(e, t, i) {
  var _a;
  ...
  if (null != l && l.length) {
    ...
    return { pyramidLevel: s4, pyramidResolution: new _({ x: u2.x, y: u2.y, spatialReference: t.spatialReference }), excessiveReading: a2 };
  }
  ...
  const h = 2 ** s4;
  return { pyramidLevel: s4, pyramidResolution: new _({ x: h * t.nativePixelSize.x, y: h * t.nativePixelSize.y, spatialReference: t.spatialReference }), excessiveReading: a2 };
}
function se(e, t) {
  const { pixelSize: n, extent: i } = e, o = v(i, t, false);
  return J2(ie(i, (n.x + n.y) / 16), t, o);
}
function ae(e, t, i) {
  var _a;
  const o = (i == null ? void 0 : i.tileSize) ?? 512, r = (i == null ? void 0 : i.alignGlobalDatasetWithAGOL) ?? true, s4 = !!(i == null ? void 0 : i.limitToSrcResolution), { extent: a2, spatialReference: l, pixelSize: c } = e, f2 = C(new _({ x: c.x, y: c.y, spatialReference: l }), t, a2);
  if (null == f2) return { projectedPixelSize: null, scales: null, srcResolutions: null, isCustomTilingScheme: false };
  ...
  let M2, w3 = x;
  const R2 = 1.001, S3 = Math.min(2, Math.max(1.414, ((_a = e.storageInfo) == null ? void 0 : _a.pyramidScalingFactor) || 2));
  if (g) {
    w3 = h;
    const e2 = t.isGeographic ? 1341104507446289e-21 : 0.29858214164761665, n = e2 * (96 * m * 39.37), i2 = t.isGeographic ? 4326 : 3857;
    M2 = C(new _({ x: e2, y: e2, spatialReference: { wkid: i2 } }), l, y), M2.x *= w3 / n, M2.y *= w3 / n;
  } else {
    M2 = { x: c.x, y: c.y };
    let e2 = 0;
  ...
  const P3 = [w3], b2 = [{ x: M2.x, y: M2.y }], G2 = 70.5310735, k2 = Math.min(G2, x) / R2;
  for (; w3 >= k2; ) w3 /= S3, M2.x /= S3, M2.y /= S3, P3.push(w3), b2.push({ x: M2.x, y: M2.y });
  if (s4) {
    const e2 = 1e-3 * c.x;
...
//# sourceMappingURL=chunk-X5IXAJYK.js.map
