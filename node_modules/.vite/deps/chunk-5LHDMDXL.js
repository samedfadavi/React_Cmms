...
async function l4(l5, o2, p4) {
  const m3 = [], u5 = { scaleInfo: t(l5), scaleExpression: null };
...
var n3 = class _n {
  static fromGraphic(t5, e3, s6, i2) {
    return new _n(t5.geometry, e3, { ...t5.attributes }, t5.visible, t5.uid, s6, i2);
  }
  ...
  update(e3, s6, i2) {
    return (this.geometry !== e3.geometry || JSON.stringify(this.attributes) !== JSON.stringify(e3.attributes) || this.symbol !== s6 || this.zOrder !== i2 || this.visible !== e3.visible) && (this.prevBounds = this.bounds, this.bounds = u(), this.zOrder = i2, this.geometry = e3.geometry, this.attributes = { ...e3.attributes }, this.symbol = s6, this.visible = e3.visible, this.symbolResource = null, this.projectedGeometry = null, true);
  }
  async projectAndNormalize(t5) {
    let n4 = this.geometry;
    if (!n4 || !n4.spatialReference || "mesh" === n4.type) return;
    "extent" === n4.type && (n4 = l2(n4)), await x(n4.spatialReference, t5);
    const p4 = a4(n4);
    if (!p4) return;
...
var b4 = class {
  ...
  async update(e3, t5, s6) {
    ...
    return await Promise.all(m3), this._index = null, new t4(o2, i2, r);
  }
  updateLevel(e3) {
  ...
  hitTest(e3, t5, i2, n4, m3) {
    ...
    for (const s6 of j2) {
      ...
      const { projectedGeometry: e4, symbolResource: t6 } = s6;
    ...
    for (const s6 of x3) {
      const { projectedGeometry: e4, symbolResource: t6 } = s6;
      if (!t6) continue;
      const { textInfo: o2, symbolInfo: i3 } = t6, r = i3.cimSymbol;
  ...
  async _updateItem(e3, t5) {
    await e3.projectAndNormalize(this._outSpatialReference), await t5(e3);
    const { size: s6 } = e3;
  ...
  _getSymbolBounds(t5, o2, r, n4, m3) {
    ...
    if (t5 || (t5 = u()), a2(t5, r), !n4 || 0 === n4[0] && 0 === n4[1] && 0 === n4[2] && 0 === n4[3]) {
      const { textInfo: t6, symbolInfo: s6 } = o2, i2 = s6.cimSymbol;
      n4 || (n4 = [0, 0, 0, 0]);
...
var F = class extends s4(S) {
  constructor(e3) {
    super(e3), this._attached = false, this._tiles = /* @__PURE__ */ new Map(), this._controller = new AbortController(), this._hashToSymbolInfo = /* @__PURE__ */ new Map(), this._lastCleanup = performance.now(), this._cleanupRequired = true, this.lastUpdateId = -1, this.renderer = null, this._updateTracking = new d3({ debugName: "GraphicsView2D" }), this.updateRequested = false, this.defaultPointSymbolEnabled = true, this._commandQueue = new t3({ process: (e4) => {
  ...
  initialize() {
    this._initAttributeStore(), this._metadata = l3.create(this.view.spatialReference), this._resourceProxy = new s5({ fetch: (e4) => Promise.all(e4.map((e5) => this.view.stage.textureManager.rasterizeItem(e5))), fetchDictionary: (e4) => {
  ...
  hitTest(e3) {
    ...
    const { resolution: t5, rotation: s6 } = this.view.state, i2 = this._graphicStore.hitTest(e3.x, e3.y, 2, t5, s6), o2 = new Set(i2), a5 = this.graphics.items.reduce((e4, t6) => (o2.has(t6.uid) && e4.set(t6.uid, t6), e4), /* @__PURE__ */ new Map());
  ...
  setHighlight(e3) {
    var _a;
    const t5 = [];
    for (const { objectId: r, highlightFlags: s6 } of e3) {
      const e4 = (_a = this._graphicStore.getItem(r)) == null ? void 0 : _a.displayId;
      t5.push({ objectId: r, highlightFlags: s6, displayId: e4 });
    }
  ...
  async _update() {
    try {
      ...
      await this._fetchResources(t5), this._write(t5);
  ...
  async _fetchResources(e3) {
    for (const { tile: t5, reader: r } of e3) {
  ...
  _write(e3) {
    for (const t5 of e3) {
      ...
      r || (r = this._createFeatureTile(t5.tile.key)), has("esri-2d-update-debug") && console.debug(`Id[${this.layerId}] Tile[${t5.tile.id}] GraphicsView onTileData`, t5), this.container.onTileData(r, { type: "update", modify: e4, remove: t5.removed, end: false, attributeEpoch: this._attributeStore.epoch }), this.container.requestRender();
    }
  }
  _writeMeshes(e3) {
    ...
    return { ...t5.serialize().message, tileId: e3.tile.id };
  }
  _setFilterState(e3) {
  ...
  async _ensureSymbolResource(e3) {
    if (!e3.symbol) return;
    const t5 = await this._getSymbolInfo(e3.symbol);
    if (!t5) return;
    const r = t5.linearCIM.filter((e4) => "text" === e4.type);
    if (r.length > 0) {
      const s6 = await this._getTextResources(e3, r);
      e3.symbolResource = { symbolInfo: t5, textInfo: s6 };
    } else e3.symbolResource = { symbolInfo: t5 };
  }
  _getSymbolInfo(e3) {
    const t5 = e3.hash();
  ...
  async _createSymbolInfo(e3, t5) {
    const r = await this._convertToCIMSymbol(t5), s6 = await this._createLinearCIM(r);
    return { hash: e3, cimSymbol: r, linearCIM: s6, meshWriters: await this._createMeshWriters(r, s6) };
  }
  async _convertToCIMSymbol(e3) {
    const t5 = W2(e3);
    if ("web-style" === t5.type) {
      return (await t5.fetchCIMSymbol()).data;
    }
    return t5;
  }
  async _createLinearCIM(e3) {
    return await Promise.all(ee.fetchResources(e3.symbol, this._cimResourceManager, [])), this.view.stage.cimAnalyzer.analyzeSymbolReference(e3, false);
  }
  async _createMeshWriters(e3, t5) {
    s(this._controller.signal);
    const r = this.container.instanceStore, s6 = this.view.stage.meshWriterRegistry, i2 = await l4(e3, t5, r);
    return Promise.all(i2.map((e4) => s6.createMeshWriter(this._storage, this._resourceProxy, { tileInfo: this.view.featuresTilingScheme.tileInfo }, e4)));
  }
  _onTileUpdate(e3) {
  ...
  async _getTextResources(e3, t5) {
    const r = new Array(), s6 = new Array();
    for (let i2 = 0; i2 < t5.length; i2++) {
      const o3 = t5[i2], { resource: a6, overrides: n5 } = o3.textRasterizationParam;
      if ((n5 == null ? void 0 : n5.length) > 0) {
        const t6 = y3.resolveSymbolOverrides({ type: "CIMSymbolReference", primitiveOverrides: n5, symbol: { type: "CIMPointSymbol", symbolLayers: [{ type: "CIMVectorMarker", enable: true, size: a6.symbol.height, anchorPointUnits: "Relative", frame: { xmin: -5, ymin: -5, xmax: 5, ymax: 5 }, markerGraphics: [{ type: "CIMMarkerGraphic", geometry: { x: 0, y: 0 }, symbol: a6.symbol, textString: a6.textString }], scaleSymbolsProportionally: true, respectFrame: true }] } }, e3, this.view.spatialReference, null, p(e3.projectedGeometry), null, null);
        t6.then((e4) => {
          const t7 = e4.symbolLayers[0], { textString: r2 } = t7.markerGraphics[0];
          s6.push({ type: "cim-rasterization-info", resource: { type: "text", textString: r2 || "", font: a6.font } }), o3.text = a6.textString = r2 || "";
        }), r.push(t6);
      } else s6.push({ type: "cim-rasterization-info", resource: a6 });
    }
    r.length > 0 && await Promise.all(r);
    const o2 = s6.map((e4) => this.view.stage.textureManager.rasterizeItem(e4)), a5 = await Promise.all(o2);
    n(a5);
    const n4 = /* @__PURE__ */ new Map();
    for (let i2 = 0; i2 < t5.length; i2++) {
      const e4 = t5[i2];
      n4.set(e4.textRasterizationParam.resource.symbol, { text: e4.text, glyphMosaicItems: a5[i2] });
    }
    return n4;
  }
  _cleanupSharedResources() {
    ...
    for (const s6 of this._graphicStore.items()) {
      const e4 = (_a = s6.symbolResource) == null ? void 0 : _a.symbolInfo.hash;
      t5.add(e4);
    }
...
e([y()], F.prototype, "_effectiveRenderer", null), e([y({ constructOnly: true })], F.prototype, "layerId", void 0), e([y({ constructOnly: true })], F.prototype, "requestUpdateCallback", void 0), e([y()], F.prototype, "container", void 0), e([y({ constructOnly: true })], F.prototype, "graphics", void 0), e([y()], F.prototype, "renderer", void 0), e([y()], F.prototype, "_updateTracking", void 0), e([y()], F.prototype, "updating", null), e([y()], F.prototype, "view", void 0), e([y()], F.prototype, "updateRequested", void 0), e([y()], F.prototype, "defaultPointSymbolEnabled", void 0), F = e([a("esri.views.2d.layers.graphics.GraphicsView2D")], F);
var V = F;

export {
  V
};
//# sourceMappingURL=chunk-5LHDMDXL.js.map
