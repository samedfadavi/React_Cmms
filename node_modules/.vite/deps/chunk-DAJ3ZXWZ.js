...
var f2 = class extends n {
  ...
  prepareRenderPasses(t2) {
    const e2 = t2.registerRenderPass({ name: "overlay", brushes: [h2.overlay], target: () => this.children, drawPhase: E.MAP });
    return [...super.prepareRenderPasses(t2), e2];
  }
  _updateMatrices(i2) {
    const { state: c } = i2, { id: p, size: m, pixelRatio: d, resolution: h3, rotation: f3, viewpoint: u, displayMat3: M2 } = c;
    if (this._viewStateId === p) return;
    const v = Math.PI / 180 * f3, g = d * m[0], _ = d * m[1];
    this._localOrigin = u.targetGeometry.clone();
    const { x: y, y: b } = this._localOrigin, x = L(y, c.spatialReference);
  ...
  _updateOverlays(t2, e2) {
    ...
    if (h3 && o2.isWrappable) {
      const t3 = a[0], e3 = a[1], l2 = 180 / Math.PI * s2, c = Math.abs(Math.cos(l2)), f3 = Math.abs(Math.sin(l2)), u = Math.round(t3 * c + e3 * f3), [M2, v] = h3.valid, g = mt(o2), { x: _, y } = n2.targetGeometry, b = [_, y], x = [0, 0];
      r2.toScreen(x, b);
      const j = [0, 0];
      let w;
      w = u > i2 ? 0.5 * i2 : 0.5 * u;
      const R = Math.floor((_ + 0.5 * g) / g), O = M2 + R * g, P = v + R * g, I = [x[0] + w, 0];
      r2.toMap(j, I), j[0] > P && (d = g), I[0] = x[0] - w, r2.toMap(j, I), j[0] < O && (d = -g), m = { worldWidth: g, xBounds: [M2, v] };
    }
...
//# sourceMappingURL=chunk-DAJ3ZXWZ.js.map
