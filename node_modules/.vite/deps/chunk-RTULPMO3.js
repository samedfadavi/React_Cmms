...
function W(n3) {
  return null == b && (b = Promise.all([_(), import("./geometryEngineBase-E6SRT7R7.js").then((n4) => n4.g), import("./hydrated-SMWXOCIS.js")])), b.then(([, r2, { hydratedAdapter: t3 }]) => {
...
function X(n3, e2) {
  if (!_3()) {
    for (const r2 of n3) if (null != r2 && !G(r2.spatialReference, e2) && N(r2.spatialReference) && N(e2) && !J(r2.spatialReference, e2)) return s2(T), { pending: W(), geometries: null };
  }
  return { pending: null, geometries: n3.map((n4) => null == n4 ? null : G(n4.spatialReference, e2) ? n4 : N(n4.spatialReference) && N(e2) ? H(n4, e2) : null) };
}
...
async function I(n3, e2, r2) {
  ...
  return null != t3 && null != e2 && await N2(t3, e2, null, r2), H(n3, e2);
}
function J(n3, e2, r2) {
...
function rn(n3, e2, r2, t3, l) {
  const { points: o2, hasZ: i, hasM: s7 } = n3, a = [], c = o2.length, f2 = [];
...
function ln(n3, e2, r2, t3, l) {
  const { paths: o2, hasZ: i, hasM: s7 } = n3, a = [];
...
function un(n3, e2, r2, t3, l) {
  const { rings: o2, hasZ: i, hasM: s7 } = n3, a = [];
  if (!cn(o2, i ?? false, s7 ?? false, e2, a, t3, l)) return null;
  const c = i || O(t3);
...
function an(n3, e2, r2, t3, l) {
  const { xmin: o2, ymin: i, xmax: s7, ymax: a, hasZ: c, hasM: f2 } = n3, p = c ? n3.zmin : l;
...
//# sourceMappingURL=chunk-RTULPMO3.js.map
