...
function* f(e3) {
  if (e3) switch (e3.type) {
    ...
    case "Polygon":
      for (const t2 of e3.coordinates) yield* t2;
      break;
    case "MultiPolygon":
      for (const t2 of e3.coordinates) for (const e4 of t2) yield* e4;
  }
}
function* p(e3, t2 = {}) {
  const { geometryType: n, objectIdField: i } = t2;
  for (const s2 of e3) {
    const { geometry: e4, properties: c3, id: l2 } = s2;
...
function E(e3, o = 4326) {
  ...
  if ("Feature" !== e3.type && "FeatureCollection" !== e3.type) throw new s("geojson-layer:unsupported-geojson-object", "missing or not supported GeoJSON object type", { data: e3 });
  const { crs: r } = e3;
  if (!r) return;
  ...
  if (!i || !s2.test(i)) throw new s("geojson:unsupported-crs", "unsupported GeoJSON 'crs' member", { crs: r });
}
function I(e3, t2 = {}) {
  const n = [], o = /* @__PURE__ */ new Set(), r = /* @__PURE__ */ new Set();
  let c3, p2 = false, y2 = null, d2 = false, { geometryType: g2 = null } = t2, m2 = false;
  for (const s2 of l(e3)) {
    const { geometry: e4, properties: t3, id: l2 } = s2;
  ...
  const h2 = c(1 === (y2 == null ? void 0 : y2.length) && y2[0] || null) ?? void 0;
  if (h2) {
  ...
  return { fields: n, geometryType: g2, hasZ: p2, objectIdFieldName: h2, objectIdFieldType: c3, unknownFields: Array.from(r) };
}
function N(e3, t2) {
...
//# sourceMappingURL=chunk-BGYJXU44.js.map
