...
var r3 = class {
  constructor(r7, s8, n11 = 0) {
    const c4 = e4 * n11 * Uint32Array.BYTES_PER_ELEMENT, h = i * n11 * s8.stride, u2 = s8.stride / 4, _4 = s8.attributes.find((t8) => "pos" === t8.name || "position" === t8.name);
    if (!_4) throw new Error("InternalError: Unable to find position attribute");
    this.layout = { ...s8, position: _4 }, this._indices = new r2(Uint32Array, c4), this._vertices = new r2(Uint32Array, h), this._metrics = new r2(Uint32Array, 0), this._metricCountOffset = this._metrics.push(0), this._strideInt = u2, this._instanceId = r7;
  }
  serialize(t8) {
    const e5 = this._indices.buffer(), i3 = this._vertices.buffer(), r7 = this._metrics.length ? this._metrics.buffer() : null;
    return t8.push(e5, i3), { instanceId: this._instanceId, layout: this.layout, indices: e5, vertices: i3, metrics: r7 };
  }
  get strideInt() {
...
var n7 = class {
  ...
  serialize() {
    ...
    return { message: { data: e5, entities: r7 }, transferList: t8 };
  }
  vertexCount() {
    var _a;
  ...
  copyLast(t8, r7) {
    ...
    const n11 = this._current.writer.indexWriter, s8 = this._current.writer.vertexWriter, o4 = this._recordIndexStart + this._recordIndexCount, c4 = this._recordVertexCount;
    ...
    const d5 = this._current.writer.layout.position, a7 = d5.packPrecisionFactor ?? 1, x3 = d5.offset / Uint32Array.BYTES_PER_ELEMENT, y3 = t8 * a7, S2 = r7 * a7;
    for (let e5 = i3 * u2; e5 <= s8.length; e5 += u2) s8.i1616Add(e5 + x3, y3, S2);
  }
  copyLastFrom(t8, e5, r7) {
    ...
    const n11 = this._current.writer.layout.stride / Uint32Array.BYTES_PER_ELEMENT, s8 = this._current.vertexStart, o4 = t8._current.vertexStart - s8, c4 = this._current.writer.indexWriter, u2 = this._current.writer.vertexWriter, h = t8._current.writer.indexWriter, _4 = t8._current.writer.vertexWriter;
    for (let f4 = t8._current.indexStart; f4 !== h.length; f4++) {
      const t9 = h.getValue(f4);
      c4.push(t9 - o4);
    }
    for (let f4 = t8._current.vertexStart * n11; f4 !== _4.length; f4++) {
      const t9 = _4.getValue(f4);
      u2.push(t9);
    }
    const d5 = this._current.writer.layout.position, a7 = d5.packPrecisionFactor ?? 1, x3 = d5.offset / Uint32Array.BYTES_PER_ELEMENT, y3 = e5 * a7, S2 = r7 * a7;
    for (let f4 = s8 * n11; f4 <= u2.length; f4 += n11) u2.i1616Add(f4 + x3, y3, S2);
    this.recordEnd();
  }
...
function r5(r7, n11, t8) {
  ...
  n11.contextTimeZone = (_a = t8.$view) == null ? void 0 : _a.timeZone;
  try {
...
function t4(e5, r7, t8, u2) {
  ...
  const o4 = e5.evaluate({ ...t8, $feature: a7 }, e5.services);
...
var c = class _c extends s6 {
  static async create(t8, s8) {
    const a7 = await o2(t8, s8.spatialReference, s8.fields), i3 = l(t8);
    return new _c(a7, i3);
  }
  constructor(e5, r7) {
...
function g(e5, r7) {
  ...
  if (t8) {
    if ("codedValue" === t8.type || "coded-value" === t8.type) {
      const r8 = e5;
      for (const e6 of t8.codedValues) if (e6.code === r8) return e6.name;
    } else if ("range" === t8.type) {
      const { max: n11, min: o4 } = m2(r7), a7 = +e5;
      if (null != o4 && null != n11 && o4 <= a7 && a7 <= n11) return t8.name;
    }
  }
  let u2 = e5;
...
var a3 = class _a extends s6 {
  static async create(t8, s8) {
    const l7 = p(t8);
    return new _a((r7) => l7.replaceAll(/{[^}]*}/g, (t9) => {
      const s9 = t9.slice(1, -1), a7 = r7.metadata.fieldsIndex.get(s9);
      if (null == a7) return t9;
      const l8 = r7.readAttribute(s9);
      return null == l8 ? "" : g(l8, a7);
    }));
  }
  constructor(e5) {
...
var r6 = class extends s6 {
  ...
  _readNormalized(e5) {
    ...
    if (null == r7) return null;
    const { normalizationField: t8, normalizationTotal: i3, normalizationType: o4 } = this._normalizationInfo, a7 = e5.readAttribute(t8);
...
var n9 = { getAttribute: (e5, r7) => e5.readAttribute(r7) };
async function o3(r7, o4) {
  try {
    const s8 = await e(r7, o4);
...
var p2 = class {
  ...
  async createComputedField(e5, s8 = false) {
    if (e5.expression) try {
      ...
      return await c.create(e5.expression, this.compilationOptions);
    } catch (d5) {
      const s9 = new s("featurelayer", "Failed to compile arcade expression", { error: d5, expression: e5.expression });
    ...
    const i3 = new s("featurelayer", "Unable to create computed field. No expression or field found", { info: e5 });
  ...
  setBounds(t8, e5, s8 = false) {
    const r7 = t8 & n6;
    if (!s8 && !this._dirtyBitset.has(t8)) return this._bounds[4 * r7] !== c3;
    this._dirtyBitset.unset(t8);
    const n11 = e5.readGeometryWorldSpace();
    if (m3(this._bounds, 4 * r7 + 4, 0), !n11 || !n11.coords.length) return this._bounds[4 * r7] = c3, this._bounds[4 * r7 + 1] = c3, this._bounds[4 * r7 + 2] = c3, this._bounds[4 * r7 + 3] = c3, false;
  ...
  getBounds(t8, e5) {
    ...
    return d(t8, i3, r7, n11, o4), i3 !== c3;
  }
  _ensureNumeric(t8, e5) {
    this._numerics[t8] || (this._numerics[t8] = []);
    m3(this._numerics[t8], e5, 0);
  }
  ...
  _ensureString(t8, e5) {
    ...
    m3(this._strings[t8], e5, null);
  }
};

// ../node_modules/@arcgis/core/views/2d/layers/features/support/FeatureMetadata.js
var i2 = class _i {
  static minimal(e5, t8, l7 = []) {
    return new _i({ geometryType: e5, fields: l7, objectIdField: t8, subtypes: null, subtypeField: null, types: null, globalIdField: null, spatialReference: null, timeInfo: null, timeReferenceUnknownClient: null, typeIdField: null });
  }
  constructor(i3) {
    this._options = i3, this._fieldsIndex = "fieldsIndex" in i3 ? Z.fromJSON(i3.fieldsIndex) : new Z(i3.fields), i3.spatialReference && (this._spatialReference = f.fromJSON(i3.spatialReference)), this._arcadeSchema = { fields: this.fieldsIndex.fields, fieldsIndex: this.fieldsIndex, geometryType: this.geometryType, objectIdField: this.objectIdField, globalIdField: this._options.globalIdField, spatialReference: this._spatialReference, timeInfo: this._options.timeInfo, typeIdField: this._options.typeIdField ?? void 0, types: this._options.types ?? void 0, subtypeField: this._options.subtypeField, subtypes: this._options.subtypes ?? void 0 };
  }
  get fieldsIndex() {
...
var l5 = class _l extends i2 {
  static create(e5) {
    return new _l({ fields: [], objectIdField: "uid", geometryType: null, spatialReference: e5, globalIdField: null, subtypeField: null, subtypes: null, timeInfo: null, typeIdField: null, types: null, timeReferenceUnknownClient: null });
  }
};

// ../node_modules/@arcgis/core/views/2d/engine/webgl/util/debug.js
var n10 = (n11, l7) => n11 && ((...n12) => l7.warn("DEBUG:", ...n12)) || (() => null);
var l6 = false;
...
var p3 = class _p {
  ...
  setTrue(t8) {
    ...
    return this._idToVisibility.forEach((t9, r7) => {
      ...
      !o4 && a7 ? e5.push(r7) : o4 && !a7 && i3.push(r7), this._idToVisibility.set(r7, a7 ? d4 | m4 : 0);
    }), { show: e5, hide: i3 };
  }
  createQuery() {
    const { geometry: t8, spatialRel: e5, where: i3, timeExtent: s8, objectIds: r7 } = this;
    return b.fromJSON({ geometry: t8, spatialRel: e5, where: i3, timeExtent: s8, objectIds: r7 });
  }
  async update(t8, e5) {
    this._hash = JSON.stringify(t8);
    const i3 = await j2(t8, null, e5);
  ...
  async _setGeometryFilter(t8) {
    ...
    const e5 = t8.geometry, i3 = t8.spatialRel || "esriSpatialRelIntersects", r7 = await v(i3, e5, this._serviceInfo.geometryType, this._serviceInfo.hasZ, this._serviceInfo.hasM);
...
var m5 = class {
  constructor(t8, e5, i3) {
    ...
    const { pixelType: s8, layout: r7, textureOnly: n11 } = e5;
  ...
  toMessage() {
    ...
    return { start: t8, end: e5, data: s8 && a7.slice(t8 * i3, (e5 + 1) * i3) || null, pixelType: r7, layout: n11 };
  }
  _initData(t8, e5) {
...
var b2 = class {
  ...
  _initialize() {
    ...
    const t8 = U.FLOAT;
    _2(`Creating AttributeStore ${y2.sharedArrayBuffer ? "with" : "without"} shared memory`), this._blockDescriptors = [{ pixelType: U.UNSIGNED_BYTE, layout: 1 }, { pixelType: U.UNSIGNED_BYTE, layout: 15, textureOnly: true }, { pixelType: U.UNSIGNED_BYTE, layout: 15, textureOnly: true }, { pixelType: t8, layout: 15 }, { pixelType: t8, layout: 15 }, { pixelType: t8, layout: 15 }, { pixelType: t8, layout: 15 }], this._blocks = this._blockDescriptors.map(() => null);
  }
  get referencesScale() {
  ...
  async update(t8, e5, s8, r7, n11 = 0) {
    const a7 = d2(this._schema, t8);
    if (this.version = n11, a7 && (has("esri-2d-update-debug") && console.debug(`Version[${n11}] AttributeStore.update`, { changed: a7 }), this._schema = t8, this._attributeComputeInfo = null, this._initialize(), null != t8)) if (s8 && (this._filters = await Promise.all(t8.filters.map((t9) => t9 ? p3.create({ geometryType: s8.geometryType, hasM: false, hasZ: false, timeInfo: s8.timeInfo, fieldsIndex: s8.fieldsIndex, spatialReference: r7 ?? s8.spatialReference, filterJSON: t9 }) : null))), "subtype" !== t8.type) this._attributeComputeInfo = { isSubtype: false, map: /* @__PURE__ */ new Map() }, await Promise.all(t8.bindings.map(async (t9) => {
      const i3 = await this._bind(e5, t9);
      ...
      for (const i3 in t8.bindings) {
        ...
        await Promise.all(s9.map(async (t9) => {
          const s10 = await this._bind(e5, t9, parseInt(i3, 10));
          this._referencesGeometry = this._referencesGeometry || ((s10 == null ? void 0 : s10.referencesGeometry()) ?? false), this._referencesScale = this._referencesScale || ((s10 == null ? void 0 : s10.referencesScale()) ?? false);
        }));
      }
    }
  }
  setHighlight(t8, e5) {
    ...
    i3.unsetComponentAllTexels(0, (1 << l4.length) - 1);
    for (const { displayId: s8, highlightFlags: r7 } of t8) {
      if (null == s8) continue;
      const t9 = f2(s8);
      i3.setComponent(0, r7, [t9]);
    }
    this._idsToHighlight.clear();
    for (const { objectId: s8, highlightFlags: r7 } of t8) this._idsToHighlight.set(s8, r7);
    for (const { objectId: s8, highlightFlags: r7 } of e5) this._idsToHighlight.set(s8, r7);
  }
  setData(t8, e5, i3, s8) {
    const r7 = f2(t8);
  ...
  setAttributeData(t8, e5, i3) {
    ...
    let c4 = null;
    r7 && (c4 = r7.isSubtype ? r7.map.get(e5.readAttribute(r7.subtypeField)) : r7.map, (c4 == null ? void 0 : c4.size) && c4.forEach((t9, r8) => {
      var _a;
      const a7 = r8 * o4 % h, c5 = Math.floor(r8 * o4 / h), d5 = this._getBlock(c5 + L.VV);
      let u2 = (_a = t9.field) == null ? void 0 : _a.read(e5, i3);
  ...
  async _bind(t8, e5, i3) {
    const s8 = await t8.createComputedField(e5), { valueRepresentation: r7 } = e5, n11 = this._attributeComputeInfo;
    if (n11.isSubtype) {
      const t9 = n11.map.get(i3) ?? /* @__PURE__ */ new Map();
      t9.set(e5.binding, { field: s8, valueRepresentation: r7 }), n11.map.set(i3, t9);
    } else n11.map.set(e5.binding, { field: s8, valueRepresentation: r7 });
    return s8;
  }
  _getInitArgs() {
    return this._initialized ? null : (this._initialized = true, this._getBlock(L.Animation), this._getBlock(L.GPGPU), { blockSize: this._size, blockDescriptors: this._blocks.map((t8) => null != t8 ? { textureOnly: t8.textureOnly, buffer: t8.buffer, pixelType: t8.pixelType } : null) });
  }
  _getBlock(t8) {
    const e5 = this._blocks[t8];
    if (null != e5) return e5;
    _2(`Initializing AttributeBlock at index ${t8}`);
  ...
  _expand() {
    if (this._size < this._schema.capabilities.maxTextureSize) {
      ...
      _2("Expanding block size to", t8, this._blocks);
  ...
  isVisible(t8) {
    return !!(this._getBlock(0).getData(t8, 0) & 1 << l4.length);
  }
  getFilterFlags(t8) {
    let e5 = 0;
...
var M2 = class {
  ...
  getAreaSimplificationThreshold(e5, t8) {
    ...
    let i3 = 0;
    return e5 > 4e3 ? i3 = T * r7 : e5 > 2e3 ? i3 = b3 * r7 : e5 > 100 ? i3 = x2 : e5 > 15 && (i3 = _3), i3;
  }
...
//# sourceMappingURL=chunk-YCDX7TWD.js.map
