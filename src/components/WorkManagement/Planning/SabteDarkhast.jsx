...
const SabteDarkhast = ({  selectedData }) => {
...
useEffect(() => {
  setRows((prevRows) => {
    // Combine previous rows with new selected data, avoiding duplicates
    const newRows = selectedData.filter(newRow => 
        !prevRows.some(existingRow => existingRow.pk_id === newRow.pk_id)
    );
    return [...prevRows, ...newRows]; // Return combined rows
}); // Update rows with the new selectedData
}, [selectedData]);
  ...
  const setSearchExpression =(tajhiz) =>
  {
   ...
   do{
      parentid=  findParentById(selectedData,String(parentid),0,idKey);
     const tajhiz2=findNodeById(selectedData,String(parentid),idKey);
      where=tajhiz2.where;
  ...
  const searchAndDisplay = () => {
    ...
    checkedNodes.forEach((row) => {
        const tajhiz= findNodeById(selectedData,row,idKey);
        if (tajhiz.Bazdid === 1) {
...
const handleSelectAll = (event) => {
  if (event.target.checked) {
    ...
    return (
      ...
      <CustomTooltip placement="top-end" title="آیتمهای انتخاب شده را از لیست درخواست کار حذف کنید"    >  
          <CustomButton>
